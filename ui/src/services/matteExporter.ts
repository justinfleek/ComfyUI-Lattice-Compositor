...
class MatteExporter {
  ...
  validateDimensions(width: number, height: number): DimensionValidation {
    ...
    return {
      ...
      message: valid
        ? undefined
        : `Adjusted to ${finalWidth}x${finalHeight} (divisible by 8)`,
    };
  }
  ...
  async generateFrame(
    ...
    for (const layer of textLayers) {
      await this.renderTextLayerToMatte(
        ctx,
        layer,
        project,
        frame,
  ...
  private async renderTextLayerToMatte(
    ...
    if (textData.pathLayerId) {
      await this.renderTextOnPathToMatte(
        ctx,
        layer,
        textData,
        project,
  ...
  private async renderTextOnPathToMatte(
    ...
    for (const char of textData.text) {
      ...
      const clampedDistance = Math.max(
        0,
        Math.min(currentDistance, totalLength),
      );

      const { point, tangent } =
        parameterizer.getPointAtDistance(clampedDistance);
