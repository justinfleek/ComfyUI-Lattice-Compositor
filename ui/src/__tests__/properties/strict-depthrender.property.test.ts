...
describe('STRICT: Depth Buffer Generation', () => {
  ...
  test.prop([
    ...
    // All values should be within clip range (empty scene = all at farClip)
    for (let i = 0; i < result.depthBuffer.length; i++) {
      expect(result.depthBuffer[i]).toBeGreaterThanOrEqual(nearClip);
      expect(result.depthBuffer[i]).toBeLessThanOrEqual(farClip);
  ...
  test.prop([
    ...
    fc.integer({ min: 64, max: 128 }),
    arbitraryCamera()
  ])('empty scene fills with farClip', (width, height, camera) => {
    const farClip = 1000;
    const options: DepthRenderOptions = {
      width,
      height,
...
describe('STRICT: Depth to ImageData', () => {
  ...
  test.prop([
    ...
    for (let i = 0; i < imageData.data.length; i++) {
      ...
      expect(Number.isInteger(imageData.data[i])).toBe(true);
    }
  });
