...
/**
 * Scale vector (positive non-zero, avoid very small values for numerical stability)
 */
...
describe('TRANSFORM Properties', () => {
  ...
  describe('Matrix4 operations', () => {
    ...
    test.prop([scaleVecArb, scaleVecArb])(
      ...
      (a, b) => {
        ...
        // Float32Array has limited precision (~7 significant digits)
        // Use 1e-5 tolerance to account for accumulated Float32 precision loss
  ...
  describe('Quaternion operations', () => {
    ...
    test.prop([
      fc.double({ min: -0.3, max: 0.3, noNaN: true }), // Keep all angles very small
      fc.double({ min: -0.1, max: 0.1, noNaN: true }), // Y axis even smaller (gimbal lock prone)
      ...
      (x, y, z) => {
        ...
        // Note: Euler angles have ambiguities, so we use loose tolerance
        // or compare by applying to a test vector
        expect(quatEqual(quat, quat2, 5e-2)).toBe(true);
      }
    );
