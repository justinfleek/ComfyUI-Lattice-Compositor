...
describe('STRICT: Export Config Validation', () => {
  test.prop([arbitraryExportConfig()])('valid config has required properties', (config) => {
    ...
    expect(typeof config.frameStart).toBe('number');
    expect(typeof config.frameEnd).toBe('number');
    expect(typeof config.width).toBe('number');
    expect(typeof config.height).toBe('number');
    expect(typeof config.fps).toBe('number');
  });

  test.prop([arbitraryExportConfig()])('frameEnd >= frameStart', (config) => {
    expect(config.frameEnd).toBeGreaterThanOrEqual(config.frameStart);
  });
...
describe('STRICT: Export Target Handling', () => {
  const validTargets: ExportTarget[] = [
    'wan-i2v', 'wan-v2v', 'wan-flf', 'svd', 
    'animatediff', 'cogvideo', 'ltx-video', 'hunyuan', 'custom'
  ];
...
describe('STRESS: Export Config Under Load', () => {
  ...
  test.prop([
    ...
    const config = {
      target: 'wan-i2v' as ExportTarget,
      frameStart: 0,
