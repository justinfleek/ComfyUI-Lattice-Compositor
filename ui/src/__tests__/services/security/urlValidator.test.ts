...
describe('URL Validator - Security', () => {
  ...
  describe('validateURL - Data URLs', () => {
    ...
    it('should BLOCK data:image/svg+xml with script tags', () => {
      const result = validateURL('data:image/svg+xml,<svg><script>alert(1)</script></svg>');
      expect(result.valid).toBe(false);
      expect(result.riskLevel).toBe('blocked');
    });
    ...
    it('should ALLOW data:image/svg+xml without script', () => {
      const result = validateURL('data:image/svg+xml,<svg><rect width="100" height="100"/></svg>');
      expect(result.valid).toBe(true);
      expect(result.riskLevel).toBe('safe');
    });
  ...
  describe('sanitizeURLForHTML', () => {
    ...
    it('should escape HTML special characters', () => {
      const result = sanitizeURLForHTML('https://example.com?foo=<bar>&x="test"');
  ...
  describe('isTrustedDomain', () => {
    ...
    it('should match exact domain', () => {
      expect(isTrustedDomain('https://example.com/path', trustedDomains)).toBe(true);
    });

    it('should match subdomain', () => {
      expect(isTrustedDomain('https://api.example.com/path', trustedDomains)).toBe(true);
      expect(isTrustedDomain('https://static.cdn.trusted.org/file', trustedDomains)).toBe(true);
    });

    it('should NOT match partial domain name', () => {
      expect(isTrustedDomain('https://fakeexample.com/path', trustedDomains)).toBe(false);
    });
