...
import { isExpressionSafe, validateProjectExpressions } from '@/services/expressions/expressionValidator';
import type { LatticeProject, Layer, SolidLayerData } from '@/types/project';
import type { AnimatableProperty, PropertyExpression } from '@/types/animation';
import type { LayerTransform } from '@/types/transform';
import type { CompositionSettings, Composition } from '@/types/project';
...
function createTransform(positionExpression?: PropertyExpression): LayerTransform {
  return {
    position: createAnimatableProperty({ x: 0, y: 0, z: 0 }, 'position', positionExpression),
...
function createSolidLayer(id: string, name: string, positionExpression?: PropertyExpression): Layer {
...
function createComposition(id: string, name: string, layers: Layer[]): Composition {
...
describeIfWorker('Expression Validator - DoS Protection', () => {
  ...
  describe('isExpressionSafe - SES Escape Attempts', () => {
    ...
    it('should BLOCK constructor escape', async () => {
      const result = await isExpressionSafe('this.constructor.constructor("return this")()');
      expect(result).toBe(false);
    }, 10000);
  });
  ...
  describe('isExpressionSafe - Valid Expressions (must ALLOW)', () => {
    ...
    it('should ALLOW bounded loop', async () => {
      const result = await isExpressionSafe('let sum=0;for(let i=0;i<10;i++)sum+=i;sum');
      expect(result).toBe(true);
    });
  ...
  describe('validateProjectExpressions', () => {
    it('should BLOCK project with malicious expression in position', async () => {
      ...
      const layer = createSolidLayer('layer1', 'Malicious Layer', maliciousExpr);
    ...
    it('should BLOCK project with recursive bomb', async () => {
      ...
      const layer = createSolidLayer('layer1', 'Recursive Layer', maliciousExpr);
      const project = createProject([layer]);
