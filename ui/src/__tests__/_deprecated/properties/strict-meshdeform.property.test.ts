...
const arbitraryPositionPin = (
  ...
  fc.record({
    ...
    stiffness: fc.double({ min: 0, max: 1, noNaN: true }),
    rotation: arbitraryAnimatableNumber(),
    scale: arbitraryAnimatableNumber(),
...
const arbitraryOverlapPin = (
  ...
  fc.record({
    ...
    position: arbitraryAnimatablePosition(width, height),
    radius: fc.integer({ min: 10, max: 200 }),
    stiffness: fc.constant(0),
    rotation: arbitraryAnimatableNumber(),
    scale: arbitraryAnimatableNumber(),
    depth: fc.integer({ min: 0, max: 100 }),
    inFront: fc.record({
      id: fc.string({ minLength: 8, maxLength: 20 }),
      name: fc.constant('inFront'),
      type: fc.constant('number' as const),
...
// Helper to create a complete WarpPin object
function createWarpPin(
  id: string,
  name: string,
  type: WarpPin['type'],
  x: number,
  ...
  const pin: WarpPin = {
    ...
    stiffness: options?.stiffness ?? (type === 'starch' ? 1.0 : 0.0),
    rotation: {
      id: `pin_rot_${id}`,
      name: 'Rotation',
      type: 'number',
  ...
  if (type === 'overlap' && options?.inFront !== undefined) {
    pin.inFront = {
      id: `pin_infront_${id}`,
      name: 'In Front',
      type: 'number',
      value: options.inFront,
      animated: false,
      keyframes: [],
    };
  }
...
describe('STRICT: Pin Trajectory Export (Wan-Move/ATI)', () => {
  test.prop([
    ...
    const pin = createWarpPin('pin1', 'Test Pin', 'position', width / 2, height / 2);
  ...
  test.prop([
    ...
    for (let i = 0; i < numPins; i++) {
      pins.push(createWarpPin(`pin_${i}`, `Pin ${i}`, 'position', i * 10, i * 10));
    }
  ...
  test.prop([
    ...
    const pin = createWarpPin('pin1', 'Out of Bounds Pin', 'position', width + 100, height + 100);
  ...
  test('visibility array matches frame count', () => {
    const pin = createWarpPin('pin1', 'Test', 'position', 50, 50);
  ...
  test('metadata is populated correctly', () => {
    const pins: WarpPin[] = [
      createWarpPin('p1', 'Pin 1', 'position', 10, 10),
      createWarpPin('p2', 'Pin 2', 'advanced', 20, 20),
      createWarpPin('p3', 'Overlap', 'overlap', 30, 30, { inFront: 50 }),
    ];
  ...
  test('exportPinsAsTrajectoryWithMetadata includes pin info', () => {
    const pins: WarpPin[] = [
      createWarpPin('p1', 'Position Pin', 'position', 10, 10),
      createWarpPin('p2', 'Rotation Pin', 'rotation', 20, 20),
    ];
  ...
  test('starch and overlap pins are filtered out', () => {
    const pins: WarpPin[] = [
      createWarpPin('p1', 'Track', 'position', 10, 10),
      createWarpPin('p2', 'Starch', 'starch', 20, 20, { stiffness: 1.0 }),
      createWarpPin('p3', 'Overlap', 'overlap', 30, 30, { inFront: 50 }),
    ];
...
describe('STRICT: Per-Frame Export Utilities', () => {
  test('exportPinPositionsPerFrame returns correct frame count', () => {
    const pin = createWarpPin('p1', 'Test', 'position', 50, 50);
  ...
  test('exportPinPositionsPerFrame has correct structure', () => {
    const pins: WarpPin[] = [
      createWarpPin('p1', 'Pin 1', 'position', 10, 20),
      createWarpPin('p2', 'Pin 2', 'rotation', 30, 40),
    ];
...
describe('STRICT: Mesh Deform Export Edge Cases', () => {
  ...
  test('handles pins at exact boundaries', () => {
    const pins: WarpPin[] = [
      createWarpPin('tl', 'TopLeft', 'position', 0, 0),
      createWarpPin('br', 'BottomRight', 'position', 99, 99),
    ];
