...
describe('interpolateCameraAtFrame', () => {
  ...
  test.prop([camera3DArb, sortedKeyframesArb])(
    ...
    (camera, keyframes) => {
      ...
      // If keyframe has position, result should match (use 5 decimal places for Float32 compatibility)
      if (kf.position) {
        expect(result.position.x).toBeCloseTo(kf.position.x, 5);
        expect(result.position.y).toBeCloseTo(kf.position.y, 5);
        expect(result.position.z).toBeCloseTo(kf.position.z, 5);
      }
    }
  );
