...
export function calculateForceField(
  ...
  switch (field.type) {
    ...
    case 'wind': {
      ...
      const gust = Math.sin(simulationTime * (field.gustFrequency ?? 0.5)) *
    ...
    case 'curl': {
      ...
      const dFdy_z = Math.sin((py + eps) * scale + time) - Math.sin((py - eps) * scale + time);
      const dFdz_y = Math.cos((pz + eps) * scale + time) - Math.cos((pz - eps) * scale + time);
      // dF/dz - dF/dx for y component
      const dFdz_x = Math.sin((pz + eps) * scale + time) - Math.sin((pz - eps) * scale + time);
      const dFdx_z = Math.cos((px + eps) * scale + time) - Math.cos((px - eps) * scale + time);
      // dF/dx - dF/dy for z component
      const dFdx_y = Math.sin((px + eps) * scale + time) - Math.sin((px - eps) * scale + time);
      const dFdy_x = Math.cos((py + eps) * scale + time) - Math.cos((py - eps) * scale + time);
    ...
    case 'orbit': {
      ...
      if (dist > 0.001) {
        ...
        const tangent = new THREE.Vector3().crossVectors(axis, centripetal).normalize();
...
export function getForceFieldTypeIndex(type: ForceFieldConfig['type']): number {
  switch (type) {
    ...
    case 'drag': return 4;
    case 'wind': return 5;
    case 'lorenz': return 6;
    case 'curl': return 7;
    case 'magnetic': return 8;
    case 'orbit': return 9;
 ...
 */
export function getFalloffTypeIndex(type: ForceFieldConfig['falloffType']): number {
  switch (type) {
    case 'none': return 0;
