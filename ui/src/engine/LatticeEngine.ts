...
export class LatticeEngine {
  ...
  /**
   * Export the scene as an encoded video
   *
   * Renders frames from startFrame to endFrame, optionally animating the camera,
   * and encodes the result to video using WebCodecs API.
   *
   * Used by export pipeline for:
   * - Uni3C: Camera motion video for render_latent
   * - TTM: Reference video showing layer motion
   *
   * @param options - Export configuration
   * @returns Encoded video blob with metadata
   */
  async exportSceneAsVideo(options: {
    startFrame: number;
    endFrame: number;
    fps: number;
    width: number;
    ...
    const originalFrame = this.state.currentFrame;
    const originalViewport = { ...this.state.viewport };

    try {
      // Resize renderer to export dimensions if different
      ...
      for (let frame = startFrame; frame <= endFrame; frame++) {
        ...
        // Apply camera animation if requested
        if (applyCamera) {
          // Sync from active camera layer if set
          if (this.activeCameraId) {
            this.syncActiveCamera();
          } else {
            // Use CameraController's own animation
            this.camera.evaluateFrame(frame);
          }
        }

        // Update orbit controls for smooth camera state
        this.camera.updateOrbitControls();
      ...
      const encodingProgressHandler = (progress: EncodingProgress) => {
        // Could expose this via another callback if needed
        engineLogger.debug(`Encoding: ${progress.percentage.toFixed(1)}%`);
      };
      ...
      this.state.currentFrame = originalFrame;

      // Restore viewport if it was changed
      if (width !== originalViewport.width || height !== originalViewport.height) {
        this.renderer.resize(originalViewport.width, originalViewport.height);
      }

      // Re-evaluate at original frame
      this.layers.evaluateFrame(originalFrame, this.audioReactiveGetter);
      if (applyCamera) {
        if (this.activeCameraId) {
          this.syncActiveCamera();
        } else {
          this.camera.evaluateFrame(originalFrame);
        }
      }
    }
  }
  ...
  /**
   * Export a single layer as a binary mask video (white layer on black background)
   *
   * Used by TTM export to create motion masks showing which pixels move.
   * Converts rendered layer to binary mask: any non-transparent pixel → white
   *
   * @param options - Export configuration
   * @returns Encoded video blob with metadata
   */
  async exportLayerAsMaskVideo(options: {
    layerId: string;
    startFrame: number;
    endFrame: number;
    fps: number;
    ...
    height: number;
    codec: 'avc' | 'vp9';
    binaryMask?: boolean; // Convert to pure white/black (default: true)
    onProgress?: (frame: number, total: number) => void;
  }): Promise<EncodedVideo> {
    this.assertNotDisposed();
    ...
    // Store visibility of all layers
    const layerVisibility = new Map<string, boolean>();
    const allLayerIds = this.layers.getLayerIds();
    ...
    try {
      ...
      // Render each frame
      for (let frame = startFrame; frame <= endFrame; frame++) {
        // Evaluate all layers (visibility already set)
        this.layers.evaluateFrame(frame, this.audioReactiveGetter);
        ...
        if (binaryMask) {
          this.convertToBinaryMask(imageData);
        }
      ...
      const encodedVideo = await encodeFrameSequence(
        frames,
        { width, height, frameRate: fps, codec, quality: 'high' },
      );
  ...
  /**
   * Convert ImageData to binary mask in-place
   * Any pixel with alpha > 0 becomes white (255,255,255,255)
   * All other pixels become black (0,0,0,255)
   */
  private convertToBinaryMask(imageData: ImageData): void {
    const data = imageData.data;
    for (let i = 0; i < data.length; i += 4) {
      // Check if pixel has any content (alpha > 0 or any color channel > 0)
      const hasContent = data[i] > 0 || data[i + 1] > 0 || data[i + 2] > 0 || data[i + 3] > 0;
