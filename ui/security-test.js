...
(async () => {
  const { isExpressionSafe } = await import('/src/services/expressions/expressionValidator.ts');

  console.log('%c=== EXPRESSION SECURITY TEST SUITE ===', 'font-size: 16px; font-weight: bold; color: #0066cc');
  ...
  const tests = [
    ...
    ['(function(){arguments.callee()})()', false, 'RECURSION', 'arguments.callee'],
    ...
    ['Array(1e6).fill(0).map(x=>x+1)', true, 'MEMORY (FAST)', 'array map 1M - fast'],
    ['[...Array(1e6)].map((_,i)=>i)', true, 'MEMORY (FAST)', 'spread+map 1M - fast'],
    ['"ab".repeat(100)', true, 'MEMORY (FAST)', 'small string repeat'],

    // This takes 18+ seconds - BLOCKED by timeout
    ['Array(1e8).fill(0)', false, 'MEMORY BOMB', 'array fill 100M - 18+ seconds'],
    ...
    // ============================================
    ['this.constructor.constructor("return this")()', false, 'SES ESCAPE', 'constructor escape'],
    ['(function(){return this})().eval', false, 'SES ESCAPE', 'this escape to eval'],
    ...
    ['(async()=>{})().constructor.constructor("return this")()', false, 'SES ESCAPE', 'async escape'],

    // ============================================
    // CATEGORY 5B: PROTOTYPE POLLUTION (must BLOCK)
    // ============================================
    ['({}).__proto__.polluted = true', false, 'PROTO POLLUTION', 'proto write'],
    ['Object.prototype.polluted = true', false, 'PROTO POLLUTION', 'Object.prototype write'],
    ['([]).__proto__.polluted = true', false, 'PROTO POLLUTION', 'Array proto write'],
    ['({}).constructor.prototype.x = 1', false, 'PROTO POLLUTION', 'constructor.prototype'],

    // ============================================
    // CATEGORY 6: REGEX DOS (must BLOCK)
    // ============================================
    ['"aaaaaaaaaaaaaaaaaaaaaaaaaaaa!".match(/(a+)+$/)', false, 'REGEX DOS', 'catastrophic backtrack'],
    ...
    ['let sum=0;for(let i=0;i<10;i++)sum+=i;sum', true, 'VALID', 'bounded for loop'],
    ['let sum=0;for(let i=0;i<100;i++)sum+=i;sum', true, 'VALID', 'bounded loop 100'],
  ...
  for (const [cat, data] of Object.entries(categories)) {
    ...
    for (const test of data.tests) {
      ...
      if (!test.passed) {
        ...
        console.log(`%c     Expected: ${test.expected}, Got: ${test.got}`, 'color: #cc0000');
      }
    }
  }
  ...
  if (results.failed === 0) {
    console.log(`%c✅ ALL TESTS PASSED: ${results.passed}/${total} (${pct}%)`,
      'font-size: 14px; color: #00aa00; font-weight: bold');
  } else {
    console.log(`%c❌ TESTS FAILED: ${results.passed}/${total} passed (${pct}%)`,
      'font-size: 14px; color: #cc0000; font-weight: bold');
    console.log('\nFailed tests:');
    for (const err of results.errors) {
      console.log(`  ❌ ${err.desc}: expected ${err.expected ? 'ALLOW' : 'BLOCK'}, got ${err.got ? 'allowed' : 'BLOCKED'}`);
