...
test.describe('Tutorial 04: Expressions - Phases 13-14 (Steps 276-355)', () => {
  ...
  test('Phase 13: Built-in Expression Functions (Steps 276-312)', async ({ page }) => {
    ...
    await h.isolateOpacity();
    await h.setExpressionText(0, 'opacity', `
      posterizeTime(1);
      time * 100
    `);
  ...
  test('Phase 14: Layer and Property References (Steps 313-355)', async ({ page }) => {
    ...
    await h.setExpressionText(1, 'position', '[thisComp.width/2, thisComp.height/2]');
    // Centers in composition

    // Step 322-324: Reference another layer
    await h.setExpressionText(1, 'position', 'thisComp.layer("Leader").position');
    // Follows Leader exactly

    // Step 325-326: Follow with offset
    await h.setExpressionText(1, 'position', 'thisComp.layer("Leader").position + [100, 0]');
    // Follows 100px to the right

    // Step 327-328: Follow with delay
    await h.setExpressionText(1, 'position', `
      var delay = 0.2;
    ...
    await h.setExpressionText(1, 'opacity', `
      var delay = index * 0.2;
    ...
    await h.setExpressionText(1, 'opacity', 'linear(time - inPoint, 0, 1, 0, 100)');
