...
class StemSeparator:
    ...
    try:
      ...
      if hasattr(torchaudio.pipelines, "HDEMUCS_HIGH_MUSDB_PLUS"):
        ...
        try:
          ...
          raise ImportError("Demucs not found. Install via: pip install demucs torchaudio") from err
  ...
  def _apply_model(
    self, waveform: "torch.Tensor", progress_callback: Callable | None = None
    ...
    for stem_name, stem_data in stems.items():
      stems[stem_name] = stem_data / weights
  ...
  def separate(
    ...
    try:
      ...
      return {
        "status": "error",
        "message": f"Missing dependency: {e!s}. Install via: pip install torch torchaudio demucs",
      }
    except Exception as e:
...
try:
  ...
  async def separate_stems_route(request):
    ...
    try:
      ...
      if not audio_b64:
        return web.json_response(
          {"status": "error", "message": "Missing 'audio' field"}, status=400
        )
      ...
      result = await loop.run_in_executor(
        None, lambda: separator.separate(audio_bytes, stems_to_return)
      )
  ...
  async def isolate_stem_route(request):
    ...
    try:
      ...
      if not stem_name:
        return web.json_response({"status": "error", "message": "Missing 'stem' field"}, status=400)
      ...
      other_stems = [s for s in stems if s != stem_name]
      ...
      if other_stems:
        ...
        for other_name in other_stems:
          ...
          with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
            ...
            temp_path = f.name
          try:
            waveform, _sr = torchaudio.load(temp_path)
