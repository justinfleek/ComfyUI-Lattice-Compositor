...
validateSaturation s = Saturation (clamp01 (ensureFinite s 0.0))

-- | Validate and create Lightness
validateLightness :: Double -> Lightness
validateLightness l = Lightness (clamp01 (ensureFinite l 0.0))

-- | Validate and create Alpha
validateAlpha :: Double -> Alpha
validateAlpha a = Alpha (clamp01 (ensureFinite a 1.0))
...
normToRGB8 r =
  let clamped = clamp01 (ensureFinite r 0.0)
...
hslToRGB (HSL (Hue h) (Saturation s) (Lightness l)) =
  let h' = h / 60.0
      ...
      r = ensureFinite (r' + m) 0.0
      g = ensureFinite (g' + m) 0.0
      b = ensureFinite (b' + m) 0.0
  in RGB
    { rgbR = normToRGB8 r
    , rgbG = normToRGB8 g
    , rgbB = normToRGB8 b
...
lerpColor c1 c2 t =
  let t' = clamp01 (ensureFinite t 0.0)
