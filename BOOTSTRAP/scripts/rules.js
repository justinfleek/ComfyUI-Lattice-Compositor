...
export const RULES = {
  nix: {
    forbidden: [
      {
        ...
        rule: "WSN-E001",
        message: "Use 'inherit (lib) x y;' instead of 'with lib;'",
      ...
      },
      {
        ...
        rule: "WSN-E002",
        message: "Use 'finalAttrs:' instead of 'rec' in mkDerivation",
      },
      {
        pattern: /rec\s*\{/,
        rule: "WSN-W001",
      ...
      },
      {
        ...
        rule: "WSN-E003",
        message: "Use lisp-case, not camelCase or snake_case",
        check: (code, filePath) => {
      ...
      },
      {
        pattern: /default\.nix/,
        rule: "WSN-E005",
        message: "Don't use default.nix in packages - use {name}.nix",
        check: (code, filePath) => {
          // Normalize path
      ...
      },
      {
        pattern: /''[\s\S]*?''/,
        rule: "WSN-E006",
        message: "Heredocs forbidden - use writeText or file imports",
      ...
      },
      {
        ...
        rule: "WSN-E007",
        message: "if/then/else in module config causes infinite recursion - use mkIf",
        context: "module",
        check: (code, filePath) => {
      ...
      },
      {
        pattern: /import\s*\(/,
        rule: "WSN-E008",
        message: "Import from derivation (IFD) forbidden - forces builds during evaluation",
        check: (code) => {
      ...
      },
      {
        ...
        rule: "WSN-E010",
        message: "Per-flake nixpkgs config forbidden - use centralized straylight-naught config",
        check: (code) => {
    ...
    required: [
      ...
      },
      {
        pattern: /_class\s*=/,
        rule: "WSN-E004",
        message: "Non-flake modules must declare '_class' attribute",
        check: (code, filePath) => {
          // Normalize path
          const normalizedPath = (filePath || '').replace(/\\/g, '/');
  ...
  haskell: {
    required: [
      {
        pattern: /import\s+Straylight\.Script/,
        rule: "STRAYLIGHT-004",
        message: "Scripts must import Straylight.Script",
        check: (code, filePath) => {
