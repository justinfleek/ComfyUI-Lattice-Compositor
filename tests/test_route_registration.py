...
# Add compositor parent directory to Python path for imports
_test_dir = os.path.dirname(os.path.abspath(__file__))
_compositor_dir = os.path.dirname(_test_dir)
_parent_dir = os.path.dirname(_compositor_dir)
...
def clear_compositor_modules():
    ...
    modules_to_remove = [k for k in list(sys.modules.keys())
                        if k.startswith('compositor') or k.startswith('lattice')]
...
class TestLatticeVectorizeRouteRegistration(unittest.TestCase):
    ...
    def test_vectorize_registers_5_routes_in_mock(self):
        ...
        """
        file_path = os.path.join(_compositor_dir, "nodes", "lattice_vectorize.py")
        module, route_count = import_module_directly(
            "lattice_vectorize", file_path, self.mock_server
        )
        ...
        self.assertEqual(
            len(routes), 5,
            f"Expected 5 routes in mock, got {len(routes)}: {routes}"
        )
...
class TestLatticeApiProxyRouteRegistration(unittest.TestCase):
    ...
    def test_api_proxy_registers_10_routes_in_mock(self):
        ...
        """
        file_path = os.path.join(_compositor_dir, "nodes", "lattice_api_proxy.py")
        module, route_count = import_module_directly(
            "lattice_api_proxy", file_path, self.mock_server
        )
        ...
        self.assertEqual(
            len(routes), 10,
            f"Expected 10 routes in mock, got {len(routes)}: {routes}"
        )
...
class TestLatticeLayerDecompositionRouteRegistration(unittest.TestCase):
    ...
    def test_layer_decomposition_registers_7_routes_in_mock(self):
        ...
        """
        file_path = os.path.join(_compositor_dir, "nodes", "lattice_layer_decomposition.py")
        module, route_count = import_module_directly(
        ...
        self.assertEqual(
            len(routes), 7,
            f"Expected 7 routes in mock, got {len(routes)}: {routes}"
        )
...
class TestLatticeFrameInterpolationRouteRegistration(unittest.TestCase):
    ...
    def test_frame_interpolation_registers_5_routes_in_mock(self):
        ...
        """
        file_path = os.path.join(_compositor_dir, "nodes", "lattice_frame_interpolation.py")
        module, route_count = import_module_directly(
...
class TestControlnetPreprocessorsRouteRegistration(unittest.TestCase):
    ...
    def test_controlnet_preprocessors_registers_5_routes_in_mock(self):
        ...
        """
        file_path = os.path.join(_compositor_dir, "nodes", "controlnet_preprocessors.py")
        module, route_count = import_module_directly(
...
class TestLatticeStemSeparationRouteRegistration(unittest.TestCase):
    ...
    def test_stem_separation_registers_4_routes_in_mock(self):
        ...
        """
        file_path = os.path.join(_compositor_dir, "nodes", "lattice_stem_separation.py")
        module, route_count = import_module_directly(
        ...
        self.assertEqual(
            len(routes), 4,
            f"Expected 4 routes in mock, got {len(routes)}: {routes}"
        )
...
class TestCompositorNodeRouteRegistration(unittest.TestCase):
    ...
    def test_compositor_node_registers_11_routes_in_mock(self):
        ...
        """
        file_path = os.path.join(_compositor_dir, "nodes", "compositor_node.py")
        module, route_count = import_module_directly(
            "compositor_node", file_path, self.mock_server
        )
        ...
        self.assertEqual(
            len(routes), 11,
            f"Expected 11 routes in mock, got {len(routes)}: {routes}"
        )
...
class TestAllModulesTotalRouteCount(unittest.TestCase):
    ...
    def test_all_modules_register_47_routes_total(self):
        ...
        for module_name, expected_count in modules:
            file_path = os.path.join(_compositor_dir, "nodes", f"{module_name}.py")
            import_module_directly(module_name, file_path, self.mock_server)

        # VERIFY TOTAL ROUTES IN MOCK
        total_routes = len(self.mock_server.routes.registered_routes)
        self.assertEqual(
            total_routes, 47,
            f"Expected 47 total routes in mock, got {total_routes}"
        )
