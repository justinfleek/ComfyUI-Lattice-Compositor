# ℵ-004: Straylight.Script — Typed Shell Infrastructure

| Field | Value |
|-------|-------|
| RFC | ℵ-004 |
| Title | Straylight.Script — Typed Shell Infrastructure |
| Author | Straylight |
| Status | Draft |
| Created | 2025-01-17 |
...
This RFC specifies **Straylight.Script**, a typed shell scripting infrastructure that replaces bash
...
### Layer 1: Straylight.Script (The Shell)
...
Key exports from `Straylight.Script`:
...
**Straylight.Script.Oci** — Container image operations

```haskell
import qualified Straylight.Script.Oci as Oci
...
```haskell
import qualified Straylight.Script.Vfio as Vfio
...
**Straylight.Script.Vm** — Virtual machine operations

```haskell
import qualified Straylight.Script.Vm as Vm
...
Census of `writeShellApplication` across straylight:
...
This isn't just a straylight problem. It's the Nix ecosystem's original sin.
...
- New scripts in straylight use Straylight.Script
- Build phases remain bash (for now) but are minimal wrappers
- Complex logic moves to compiled Haskell called from phases

### The Long Game

If Straylight.Script proves itself, the pattern can spread:

1. **straylight scripts** — fully typed (this RFC)
1. **straylight build phases** — minimal bash calling compiled tools
...
Code is **Straylight.Script conformant** if it:

1. Uses `Straylight.Script` for all shell operations (no raw `System.Process`)
...
```haskell
module Straylight.Packages.Nvidia where
...
```haskell
module Straylight.Packages.MyProject where
...
```haskell
module Straylight.Scripts.Deploy where

import Straylight.Script
import qualified Straylight.Packages.Nvidia as Nvidia
...
┌─────────────────────────────────────────────────────────────────┐
│  Nix Evaluation                                                 │
│  straylight.eval "Module.name" { args }                              │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
| 7 | Incremental adoption (straylight.phases.interpret) | **Next** |
...
**Solution**: `straylight.phases.interpret` — a bridge that lets traditional Nix
...
1. `straylight.phases.interpret` takes a list of typed actions
...
- `Straylight.Nix.Tools.Jq` — jq queries
- `Straylight.Nix.Tools.PatchElf` — rpath manipulation
- `Straylight.Nix.Tools.Install` — typed install with correct modes
- `Straylight.Nix.Tools.Substitute` — substituteInPlace
...
1. Implement `straylight.phases.interpret` bridge
1. Add `Straylight.Nix.Tools.Wrap` module
1. Add `Chmod` action
...
- [ℵ-003: The Straylight Prelude](straylight-003-prelude.md)
- [ℵ-007: Nix Formalization](straylight-007-formalization.md)
