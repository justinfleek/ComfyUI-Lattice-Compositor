...
module Straylight.Nix.Syntax (
    ...
    module Straylight.Script.Tools.CMake,
) where

import Straylight.Nix.Derivation (
    Action (..),
...
import qualified Straylight.Nix.Derivation as D
import Straylight.Nix.Types (PkgRef (..), StorePath (..))
import Straylight.Script.Tools.CMake (BuildType (..), Options (..), buildArgs, defaults)
import qualified Straylight.Script.Tools.CMake
...
data Attr
    ...
    | CheckInputsA [Text]
    | PatchesA [StorePath]
    | CMakeA Straylight.Script.Tools.CMake.Options -- typed cmake
...
@
-}
cmake :: Straylight.Script.Tools.CMake.Options -> Attr
cmake = CMakeA
...
-- | Change file permissions (relative to $out)
-- Example: chmod "bin/mytool" 0o755
chmod :: Text -> Int -> Action
chmod = Chmod
...
{- | Reference a build-time package (for cross-compilation).

The package is automatically added to nativeBuildInputs from buildPackages.

@
preConfigure
    [ buildTool "cmake" ["-S", ".", "-B", "build"]
    , buildTool "pkg-config" ["--cflags", "openssl"]
    ]
@

Use this for build-time tools that must run on the build platform,
not the host platform (e.g., when cross-compiling).
-}
buildTool :: Text -> [Text] -> Action
...
{- | Specify multiple outputs for a package.

@
outputs ["out", "dev", "lib"]
@

Default is ["out"]. Common outputs:
- "out": Default output (binaries, data)
- "dev": Development files (headers, pkg-config)
- "lib": Libraries only
- "bin": Binaries only

Use 'outPathNamed' in actions to reference specific outputs:
@
installPhase
    [ Install.header "include/foo.h" "dev:include/foo.h"  -- → \$dev/include/foo.h
    , Install.lib "libfoo.so" "lib:lib/libfoo.so"         -- → \$lib/lib/libfoo.so
    , Install.bin "foo" "out:bin/foo"                      -- → \$out/bin/foo
    ]
@
-}
outputs :: [Text] -> Attr
...
mkDerivation = foldr apply defaultDrv
  where
    ...
    apply (OutputsA outNames) d =
        d{drvOutputs = outNames}
