...
@
-}
module Straylight.Nix.Derivation (
    -- * Core types
    Drv (..),
    defaultDrv,
...
data Action
    ...
    | {- | Change file permissions.
      @Chmod "bin/mytool" 0o755@ → chmod 755 $out/bin/mytool
      -}
      Chmod !Text !Int
    deriving (Eq, Show, Generic)
...
When actions fail at build time, this maps the error back to the
Haskell source location where the action was defined.
-}
data Location = Location
    { locFile :: !Text
    -- ^ Source file path (e.g., "packages/zlib.hs")
    , locLine :: !Int
    -- ^ Line number (1-based)
    , locColumn :: !Int
...
location file line = Location file line 1

-- | Create a location from file, line, and column.
locationWithColumn :: Text -> Int -> Int -> Location
locationWithColumn = Location
...
defaultDrv =
    Drv
        ...
        , drvOutputs = ["out"] -- default single output
        }
...
actionToNix (Remove path) = do
    ...
    mkAttrs (Map.fromList pairs)
actionToNixBase (Unzip destDir) = do
    pairs <-
        sequence
...
actionToNix (ToolRun pkgRef args) = do
    ...
    pairs <-
        sequence
            ...
            , ("pkg",) <$> mkString pkgName
            , ("buildPackage",) <$> mkBool isBuildPkg
            , pure ("args", argsVal)
            ]
    mkAttrs (Map.fromList pairs)
