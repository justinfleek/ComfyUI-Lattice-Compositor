...
{- |
Module      : Straylight.Script.Getopt
...
-- | Names exported by Straylight.Script that might conflict with command names
...
generateModule moduleName cmdName help =
    let opts = filter (not . isBuiltinOpt) (helpOptions help)
        ...
        -- Build list of names to hide from Straylight.Script
        ...
        importLine =
            if null hideList
                then "import Straylight.Script"
                else "import Straylight.Script hiding (" <> T.intercalate ", " hideList <> ")"
     in T.unlines
            [ "{-# LANGUAGE OverloadedStrings #-}"
            , "{-# LANGUAGE RecordWildCards #-}"
            , "-- |"
            , "-- Module      : Straylight.Script.Tools." <> moduleName
