...
#
#     outputs = { self, straylight, buck2-prelude, ... }:
#       straylight.lib.mkFlake { inherit inputs; } {
#         imports = [ straylight.modules.flake.build ];
#
...
let
  ...
  cfg = config.straylight-naught.build;
in
{
  _class = "flake";
  ...
  options.straylight-naught.build = options.build;
  ...
  config = lib.mkIf cfg.enable {
    ...
    # ──────────────────────────────────────────────────────────────────────────
    straylight-naught.nixpkgs.overlays = lib.mkBefore [
