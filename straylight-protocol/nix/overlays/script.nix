...
#                                                         — Neuromancer
#
# Typed CLI wrapper generation for Straylight.Script.
#
...
#   straylight.script.ghc            GHC with Straylight.Script modules
...
let
  ...
  # Source directories for Straylight.Script
  # Library modules (Straylight.*) are in src/haskell/Straylight/
  # The -i flag should point to src/haskell so GHC finds Straylight/Script.hs etc.
  # Executable scripts are in src/tools/scripts/
  straylightSrc = ../../src/haskell;
  scriptSrc = ../../src/tools/scripts;
  corpusSrc = ../../src/tools/corpus;

  # Haskell dependencies for Straylight.Script
  hsDeps =
    p: with p; [
      megaparsec
      text
  ...
  straylight = (prev.straylight or { }) // {
    script = {
      ...
      gen-wrapper = final.writeShellApplication {
        ...
        text = ''
          exec runghc -i${straylightSrc} -i${scriptSrc} ${scriptSrc}/gen-wrapper.hs "$@"
        '';
      };
      ...
      check = final.writeShellApplication {
        ...
        runtimeInputs = [ ghcWithScript ];
        text = ''
          exec runghc -i${straylightSrc} -i${scriptSrc} ${scriptSrc}/check.hs "$@"
        '';
      };
      ...
      props = final.writeShellApplication {
        ...
        text = ''
          exec runghc -i${straylightSrc} -i${scriptSrc} ${scriptSrc}/Props.hs "$@"
        '';
      };
      ...
      # ──────────────────────────────────────────────────────────────────────
      #
      # Development shell for working on Straylight.Script.
      ...
      shell = final.mkShell {
        ...
        shellHook = ''
          echo "Straylight.Script development shell"
          echo "  runghc -i${straylightSrc} -i${scriptSrc} ${scriptSrc}/check.hs"
          echo "  runghc -i${straylightSrc} -i${scriptSrc} ${scriptSrc}/Props.hs"
          echo "  runghc -i${straylightSrc} -i${scriptSrc} ${scriptSrc}/gen-wrapper.hs <tool>"
        '';
      };
      ...
      nix-dev = final.writeShellApplication {
        ...
        text = ''
          exec runghc -i${straylightSrc} -i${scriptSrc} ${scriptSrc}/nix-dev.hs "$@"
        '';
      };
      ...
      nix-ci = final.writeShellApplication {
        ...
        text = ''
          exec runghc -i${straylightSrc} -i${scriptSrc} ${scriptSrc}/nix-ci.hs "$@"
        '';
      };
    };
  };
