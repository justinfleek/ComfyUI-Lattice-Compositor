...
LEAN_FILES=$(find "$PROJECT_ROOT/leancomfy/lean" -name "*.lean" 2>/dev/null | grep -v ".lake\|build" | head -20)
...
if grep -rn --include="*.lean" "\bsorry\b" "$PROJECT_ROOT/leancomfy/lean" 2>/dev/null | grep -v ".lake\|build\|test" | head -20 | while read -r line; do
...
if grep -rn --include="*.lean" "unsafe" "$PROJECT_ROOT/leancomfy/lean" 2>/dev/null | grep -v "import\|test\|\.lake\|build\|--.*unsafe" | head -20 | while read -r line; do
