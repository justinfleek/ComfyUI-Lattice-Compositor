...
if grep -r --include="*.lean" "sorry" "$PROJECT_ROOT/standalone/lattice-core/lean" 2>/dev/null | grep -v "^Binary" | grep -v "^\\./" | grep -v "TODO.*sorry\|--.*sorry\|/\*.*sorry"; then
  echo "❌ ERROR: Found 'sorry' without TODO comment in Lean4 code"
  grep -rn --include="*.lean" "sorry" "$PROJECT_ROOT/standalone/lattice-core/lean" 2>/dev/null | grep -v "^Binary" | grep -v "^\\./" | grep -v "TODO.*sorry\|--.*sorry\|/\*.*sorry"
  VIOLATIONS=$((VIOLATIONS + 1))
fi

# Check for unproven 'axiom' (without justification comment)
if grep -r --include="*.lean" "^\s*axiom\s" "$PROJECT_ROOT/standalone/lattice-core/lean" 2>/dev/null | grep -v "^Binary" | grep -v "^\\./" | grep -vE -- "--.*axiom|/\*.*axiom"; then
...
if grep -r --include="*.lean" "instance.*Extractable" "$PROJECT_ROOT/standalone/lattice-core/lean" 2>/dev/null | grep -v "^Binary" | grep -v "^\\./" ; then
