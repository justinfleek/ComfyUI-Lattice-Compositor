...
calculateTimeStretch videoFps compFps =
  let
    safeVideoFps = ensureFinite videoFps 16.0
    safeCompFps = ensureFinite compFps 16.0
...
calculateVideoFrameCount duration fps =
  let
    safeDuration = ensureFinite duration 0.0
    safeFps = ensureFinite fps 16.0
...
checkFpsMismatch videoFps compFps threshold =
  let
    safeVideoFps = ensureFinite videoFps 16.0
    safeCompFps = ensureFinite compFps 16.0
    safeThreshold = ensureFinite threshold 0.5
  in
