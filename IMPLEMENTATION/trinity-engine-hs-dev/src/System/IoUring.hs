...
import GHC.Conc (getNumCapabilities, threadCapability, myThreadId)
...
import Foreign (Ptr, nullPtr, castPtr, plusPtr, mallocBytes, free, poke)
import Foreign.Storable (pokeByteOff)
import Foreign.C.String (CString)
import Foreign.C.Error (Errno(Errno))
import System.Posix.Types (Fd(Fd), ByteCount, FileOffset)
...
initIoUring (IOCtxParams batchSize concurrency) = do
  ...
  caps <- V.generateM numCaps $ \idx -> do
    ...
    return $ CapCtx idx uring batchSize concurrency
  return $ IOCtx caps

closeIoUring :: IOCtx -> IO ()
closeIoUring (IOCtx caps) = V.forM_ caps closeCapCtx
  where
    closeCapCtx (CapCtx _ uring _ _) = URing.closeURing uring
...
submitBatch ctx batchPrep = do
  ...
  where
    ...
    submitSmallBatch (IOCtx caps) ops = mask_ $ do
      -- Pick the capability based on current thread
      (capIdx, _) <- threadCapability =<< myThreadId
      -- Handle case where capIdx might be out of bounds if caps size < num capabilities (unlikely if init correct)
      let safeIdx = capIdx `mod` V.length caps
      let cap = caps V.! safeIdx
          uring = _capURing cap
      ...
      results <- V.generateM nOps $ \_ -> do
        ...
        -- In a real implementation we would match the completion ID with the Op ID
        -- For this simple "wait for all" approach, we assume FIFO or mapped
...
ioCtxParams (IOCtx caps) = 
  if V.null caps 
    ...
    else let CapCtx _ _ batchSize concurrency = V.head caps
