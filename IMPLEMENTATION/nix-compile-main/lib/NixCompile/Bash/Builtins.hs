...
-- | Look up the expected type for a command flag argument
...
-- | Look up the expected type for a positional argument
-- Position is 0-based
lookupPositionalType :: Text -> Int -> Maybe Type
lookupPositionalType cmd pos = do
  schema <- Map.lookup cmd builtins
  let positionals = cmdPositional schema
  if pos < length positionals
    then Just (argType (positionals !! pos))
    else Nothing
