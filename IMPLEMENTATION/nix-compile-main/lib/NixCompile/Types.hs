...
-- | The type language for bash environment inference
-- Note: We always infer concrete types (TInt, TBool, TString, TPath).
-- Type variables (TVar) are used during unification and resolved to concrete types.
data Type
  = TInt
  | TString
  | TBool
...
-- | Constraint with source location for error reporting
-- This is a separate type to avoid breaking existing code
data LocatedConstraint = LocatedConstraint
...
data Fact
  ...
  | -- | Command invocation with positional arg
    CmdPositional !Text !Int !Text !Span -- cmd, position (0-based), varname, span
  | -- | Store path usage
