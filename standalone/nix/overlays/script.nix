...
in
{
  aleph = (prev.aleph or { }) // {
    script = {
      ...
      compiled = {
        # Validation scripts - migrated from bash to Haskell
        check-file-size = mk-compiled-script {
          name = "check-file-size";
          deps = [ ];
        };

        check-banned-patterns = mk-compiled-script {
          name = "check-banned-patterns";
          deps = [ final.ripgrep ];
        };

        check-type-escapes = mk-compiled-script {
          name = "check-type-escapes";
          deps = [ final.ripgrep ];
        };

        check-proofs = mk-compiled-script {
          name = "check-proofs";
          deps = [ final.ripgrep ];
        };

        check-error-messages = mk-compiled-script {
          name = "check-error-messages";
          deps = [ final.ripgrep ];
        };

        setup-database = mk-compiled-script {
          name = "setup-database";
