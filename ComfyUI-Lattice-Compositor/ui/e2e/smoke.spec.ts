...
test.describe("Lattice Compositor Smoke Test", () => {
  ...
  test("opens in fullscreen mode by default", async ({ page }) => {
    ...
    if (await compositorTab.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      await videoIcon.isVisible({ timeout: 2000 }).catch(() => false)
    ) {
    ...
    if (found) {
      ...
      if (panelBox && viewport) {
        ...
        console.log(
          `Panel width: ${panelBox.width}px (${(widthRatio * 100).toFixed(1)}% of viewport)`,
        );
      ...
      if (await closeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        await page.screenshot({
          path: "e2e/screenshots/04-fullscreen-verified.png",
        });
        ...
        const stillFullscreen = await fullscreenPanel
          .isVisible()
          .catch(() => false);
      ...
      await page.screenshot({
        path: "e2e/screenshots/03-not-found.png",
        fullPage: true,
      });
    }
  });
  ...
  test("compositor UI elements are visible when opened", async ({ page }) => {
    ...
    const compositorTab = page
      .locator('[title*="Motion" i], [title*="Lattice" i], .pi-video')
      .first();
    ...
    if (await compositorTab.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const toolbar = page
        .locator('[class*="toolbar" i], [class*="Toolbar"]')
        .first();
      const timeline = page
        .locator('[class*="timeline" i], [class*="Timeline"]')
        .first();
