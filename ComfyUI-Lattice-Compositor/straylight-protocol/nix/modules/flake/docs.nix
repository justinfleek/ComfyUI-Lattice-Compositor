...
{ config, lib, ... }:
let
  cfg = config.straylight-naught.docs;
in
{
  _class = "flake";

  options.straylight-naught.docs = {
  ...
  config = lib.mkIf cfg.enable {
    perSystem =
      ...
      let
        ...
        docs-options-drv =
          if cfg.modules == [ ] || !has-ndg then
            ...
            let
              ...
              '';
            in
            pkgs.runCommand "straylight-naught-docs-options"
              {
        ...
        docs-prelude-drv =
          pkgs.runCommand "straylight-naught-docs-prelude"
            {
      ...
      in
      {
        packages = {
          ...
          docs-prose =
            pkgs.runCommand "straylight-naught-docs-prose"
              {
          ...
          docs = pkgs.runCommand "straylight-naught-docs" { } ''
            mkdir -p $out
