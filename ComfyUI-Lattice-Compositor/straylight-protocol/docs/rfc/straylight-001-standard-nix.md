...
1. **Centralized** — `straylight` owns the base overlays; projects extend

### 3. Central nixpkgs Configuration

All flakes consuming `straylight` SHALL use its nixpkgs configuration:

```nix
perSystem = { system, ... }: {
  _module.args.pkgs = inputs.straylight.nixpkgs.${system};
};
```
...
in [ℵ-002](straylight-002-lint.md). CI pipelines SHALL fail on `wsn-lint` errors.

## Conformance

A flake is **Straylight Standard Nix conformant** if it:

1. Imports `straylight` and uses its centralized nixpkgs configuration
