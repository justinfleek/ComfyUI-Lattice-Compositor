...
module Lattice.Utils.FpsUtils
  ...
  , calculateFrameCount
    -- * Assertions
  , assertValidFps
    -- * Plain Double versions (compatibility)
  , validateFpsD
  , calculateDurationD
  ) where
...
-- | Validate fps and return as Double
validateFpsD :: Maybe Double -> Double -> Double
validateFpsD Nothing fallback = fallback
validateFpsD (Just f) fallback
  | not (isFiniteDouble f) || f <= 0 = fallback
  | otherwise = max (fromIntegral minFps) (min (fromIntegral maxFps) f)

-- | Calculate duration returning Double
calculateDurationD :: Double -> Double -> Double
calculateDurationD frameCount fps
  | fps <= 0 = 0
  | isFiniteDouble result = result
  | otherwise = 0
  where result = frameCount / fps
