...
-- | Export clampD as C function
foreign export ccall "clampD"
...
c_clamp jsonInput = do
  ...
  case decode (BSL.fromStrict inputBS) :: Maybe ClampInput of
    Nothing -> jsonToCString $ errorResponse (T.pack "Invalid JSON input for clampD")
    Just input -> do
      let result = clampD (cValue input) (cMinVal input) (cMaxVal input)
      jsonToCString $ successResponse (toJSON result)
