...
safeListGet lst idx fallback
  | idx < 0 || idx >= length lst = fallback
  | otherwise = lst !! idx

-- | Clamp a value to [0, 1]
clamp01D :: Double -> Double
clamp01D x = max 0 (min 1 x)
  ...
  | otherwise =
      let clampedFrame = frame
          getArrayValue arr idx =
            let val = safeListGet arr idx 0.0
    ...
    FeatureOnsets ->
      let onsets = audioAnalysisOnsets analysis
          onsetVal = safeListGet onsets clampedFrame 0
