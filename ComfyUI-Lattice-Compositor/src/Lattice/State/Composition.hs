...
-- Note: calculateDurationD is imported from Lattice.Utils.FpsUtils
  ...
  let
    ...
    fps = case (mPartialSettings, mActiveSettings) of
      (Just (CompositionSettings _ _ _ fpsVal _ _ _ _ _ _ _ _), _) ->
        if fpsVal > 0 then validateFpsD (Just fpsVal) defaultFpsD else getFpsFromActive
      (_, Just (CompositionSettings _ _ _ fpsVal _ _ _ _ _ _ _ _)) ->
        if fpsVal > 0 then validateFpsD (Just fpsVal) defaultFpsD else defaultFpsD
      _ -> defaultFpsD
      where
        getFpsFromActive = case mActiveSettings of
          Just (CompositionSettings _ _ _ activeFps _ _ _ _ _ _ _ _) ->
            if activeFps > 0 then validateFpsD (Just activeFps) defaultFpsD else defaultFpsD
          Nothing -> defaultFpsD
    ...
    duration = calculateDurationD frameCount fps
