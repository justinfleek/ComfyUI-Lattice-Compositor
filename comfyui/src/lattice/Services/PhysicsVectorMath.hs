...
vec2Create x y = Vec2 (ensureFinite x 0.0) (ensureFinite y 0.0)
...
vec2Add a b =
  Vec2 (ensureFinite (vec2X a + vec2X b) 0.0) (ensureFinite (vec2Y a + vec2Y b) 0.0)
...
vec2Length v =
  ...
  in ensureFinite (safeSqrt lengthSq) 0.0

-- | Calculate squared vector length (for performance, avoids sqrt)
-- Pure function: same inputs → same outputs
vec2LengthSq :: Vec2 -> Double
vec2LengthSq v = ensureFinite (vec2X v * vec2X v + vec2Y v * vec2Y v) 0.0
...
vec2Dot a b = ensureFinite (vec2X a * vec2X b + vec2Y a * vec2Y b) 0.0
...
vec2Rotate v angle =
  let finiteAngle = ensureFinite angle 0.0
