...
-- Pure vector operations: add, sub, mul, div, normalize, dot, cross, length, clamp
...
vectorNormalize vec
  | null vec = vec
  | otherwise =
      let len = safeSqrt (sum (map (\v -> v * v) vec)) 0.0
