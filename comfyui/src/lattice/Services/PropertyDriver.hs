...
applyTransform transform value mPrevSmoothed = case transform of
  TransformScale factor ->
    let finiteFactor = ensureFinite factor 1.0
    in (ensureFinite (value * finiteFactor) value, mPrevSmoothed)
  
  TransformOffset amount ->
    let finiteAmount = ensureFinite amount 0.0
    in (ensureFinite (value + finiteAmount) value, mPrevSmoothed)
  ...
  TransformClamp mMin mMax ->
    ...
    in (ensureFinite clamped value, mPrevSmoothed)
  
  TransformSmooth smoothing ->
    let finiteSmoothing = ensureFinite smoothing 0.5
    ...
    in (ensureFinite smoothed value, Just smoothed)
  
  TransformInvert ->
    (ensureFinite (1.0 - value) value, mPrevSmoothed)
  
  TransformRemap inMin inMax outMin outMax ->
    let finiteInMin = ensureFinite inMin 0.0
        finiteInMax = ensureFinite inMax 1.0
        finiteOutMin = ensureFinite outMin 0.0
        finiteOutMax = ensureFinite outMax 1.0
        inRange = finiteInMax - finiteInMin
    in if inRange == 0.0
       then (ensureFinite ((finiteOutMin + finiteOutMax) / 2.0) value, mPrevSmoothed)
       else
         let normalized = (value - finiteInMin) / inRange
             remapped = finiteOutMin + normalized * (finiteOutMax - finiteOutMin)
         in (ensureFinite remapped value, mPrevSmoothed)
  
  TransformThreshold threshold ->
    let finiteThreshold = ensureFinite threshold 0.5
        result = if value > finiteThreshold then 1.0 else 0.0
    in (ensureFinite result value, mPrevSmoothed)
  ...
  TransformOscillate freq amp phase ->
    ...
    in (ensureFinite oscillated value, mPrevSmoothed)
...
blendValue base driven mode amount =
  let finiteAmount = max 0.0 (min 1.0 (ensureFinite amount 1.0))
      result = case mode of
        BlendReplace -> driven
