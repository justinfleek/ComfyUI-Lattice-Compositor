...
-- |
-- | Pure types for export pipeline orchestration.
-- | No FFI - all rendering done by Haskell backend via Bridge.
-- |
...
-- | Layer data for export (pure representation)
type LayerData =
  { id :: String
  , name :: String
  , visible :: Boolean
  ...
  , imageData :: Maybe String    -- Base64 encoded image
  }

-- | Camera keyframe data (pure representation)
type CameraKeyframeData =
  { frame :: Int
...
instance EncodeJson ExportStage where
...
instance EncodeJson ControlType where
  encodeJson = genericEncodeJson
