...
module Lattice.Services.Blur.StackBlur
  ( mulTableValue
  , shgTableValue
  , stackDivisor
  , pixelWeight
...
-- | Get multiplication table value for radius.
mulTableValue :: Int -> Int
mulTableValue radius =
  fromMaybe 512 (index mulTable radius)

-- | Get shift table value for radius.
shgTableValue :: Int -> Int
shgTableValue radius =
  fromMaybe 24 (index shgTable radius)
...
stackDivisor :: Int -> Int
stackDivisor radius =
  let r1 = radius + 1
  in r1 * r1

-- | Calculate weighted contribution of a pixel at distance from center.
pixelWeight :: Int -> Int -> Int
pixelWeight radius distance =
  let absDistance = if distance < 0 then negate distance else distance
  in if absDistance > radius
     then 0
     else radius + 1 - absDistance

-- | Apply StackBlur division using lookup tables.
stackDivide :: Int -> Int -> Int
stackDivide sumVal radius =
  let mul = mulTableValue radius
      shg = shgTableValue radius
  in (sumVal * mul) `shr` shg
...
clampIndex idx maxIndex
  | idx < 0 = 0
  | idx > maxIndex = maxIndex
  | otherwise = idx
...
stackSize :: Int -> Int
stackSize radius = radius * 2 + 1

-- | Calculate the index offset for stack initialization.
stackOutOffset :: Int -> Int
stackOutOffset radius = radius
