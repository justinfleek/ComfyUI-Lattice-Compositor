...
module Lattice.Services.Video.Transitions
  ...
  , blendAdd
  , blendSubtract
    -- * Transition Rendering (Pure - returns request for Bridge)
  , renderTransition
...
-- |
-- | This replaces the FFI-based rendering. The actual rendering is performed
-- | by the Haskell backend when this request is sent via Bridge.
type TransitionRequest =
  { fromCanvasId :: String   -- Source canvas element ID
  , toCanvasId :: String     -- Target canvas element ID
  , progress :: Number       -- Eased progress (0-1)
  , blendMode :: String      -- Blend mode string
  , softness :: Number       -- Edge softness (0-1)
  , centerX :: Number        -- Center X for radial effects (0-1)
  , centerY :: Number        -- Center Y for radial effects (0-1)
  , seed :: Int              -- Seed for dissolve randomization
  }
...
-- | Create a transition request from state
-- |
-- | This is a pure function that creates a TransitionRequest data structure.
-- | The actual rendering is performed by the Haskell backend when this
-- | request is sent via Bridge.
-- |
-- | @param state The current transition state
-- | @returns A TransitionRequest that can be sent to the backend
renderTransition :: TransitionState -> TransitionRequest
renderTransition state =
  ...
  in { fromCanvasId: state.fromCanvasId
...
-- | Exponentiation using Data.Number.pow (pure, no FFI)
