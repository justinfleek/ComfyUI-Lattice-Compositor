...
bool segment_triangle_intersection(const Vec3d& x0, size_t /*index0*/,
                                   ...
                                   bool /*verbose*/ )
{
    static int initialized_exact = 0;
    if (initialized_exact == 0) exactinit();
    initialized_exact++;
    double bary[5];
    
    bool return_value1,return_value2;
    
    auto start = std::chrono::high_resolution_clock::now();
    auto end = std::chrono::high_resolution_clock::now();
    
    static long long bridson = 0;
    static long long shewchuk = 0;
    
    start = std::chrono::high_resolution_clock::now();
    return_value2 = simplex_intersection3d_shewchuk( 2, x0.v, x1.v, x2.v, x3.v, x4.v, &bary[0], &bary[1], &bary[2], &bary[3], &bary[4] ) != 0;
    end = std::chrono::high_resolution_clock::now();
    shewchuk += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
    
    /*start = std::chrono::high_resolution_clock::now();
    return_value1 = simplex_intersection3d( 2, x0.v, x1.v, x2.v, x3.v, x4.v, &bary[0], &bary[1], &bary[2], &bary[3], &bary[4] ) != 0;
    end = std::chrono::high_resolution_clock::now();
    bridson += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();*/
    
   /*if (initialized_exact % 1000000 == 0) {
        std::cout << "bridson: " << bridson << '\n';
        std::cout << "shewchuk: " << shewchuk << '\n';
        //std::getchar();
    }*/
