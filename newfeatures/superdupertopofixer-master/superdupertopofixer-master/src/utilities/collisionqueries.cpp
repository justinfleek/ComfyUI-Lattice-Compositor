...
// find distance between 0 and 1-2-3, with barycentric coordinates for closest point (s1,s2,s3), and
// a normal that points from 1-2-3 towards 0 (unreliable if distance==0 or very small)

// check_point_triangle_proximity( m_surface.get_position(v),m_surface.get_position(tri[0]),m_surface.get_position(tri[1]),
//                                 m_surface.get_position(tri[2]),distance, s1, s2, s3, normal );
