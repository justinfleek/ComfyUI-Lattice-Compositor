/*
    Code built from the LosTopos library, developed and published by Fang Da, Christopher Batty, and Eitan Grinsprun,
    see: http://www.cs.columbia.edu/cg/multitracker/
*/
#ifndef VEC_H
#define VEC_H
...
// ---------------------------------------------------------
    ...
    { for(unsigned int i=0; i<N; ++i) v[i]=value_for_all; }     //eg Vec3d(1.5) means {1.5,1.5,1.5};
...
typedef Vec<4, long long>     Vec4ll;
...
{
    T l=LosTopos::sqr(a.v[0]);
    for(unsigned int i=1; i<N; ++i) l+=LosTopos::sqr(a.v[i]);
    return l;
}
...
{ 
    T d=LosTopos::sqr(a.v[0]-b.v[0]);
    for(unsigned int i=1; i<N; ++i) d+=LosTopos::sqr(a.v[i]-b.v[i]);
    return d;
}
...
inline unsigned int hash(const Vec<N,T> &a)
{
    ...
    for(unsigned int i=1; i<N; ++i)
        h=LosTopos::hash(h ^ a.v[i]);
    return h;
}

template<unsigned int N, class T>
inline unsigned int hash_value(const Vec<N,T> &a) {
    return hash(a);
}
