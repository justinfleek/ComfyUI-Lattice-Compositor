...
#if defined(__GNUC__) || \
	defined(__clang__)  // use inline assembly, Gnu/AT&T syntax
	int a, b, c, d;
	__asm("cpuid"
	      : "=a"(a), "=b"(b), "=c"(c), "=d"(d)
	      : "a"(leaf), "c"(subleaf)
...
// Parameter:
// logical_processors: an optional pointer to an integer that will receive the
// number of logical processors. Return value: number of physical processors
inline int physicalProcessors(int* logical_processors = 0) {
	...
	bool hyperthreadingSupported =
		false;  // CPU supports hyperthreading / simultaneous multithreading
	int systemProcessors =
		std::thread::hardware_concurrency();  // number of processors reported
	                                          // by operating system
	...
	if (vendor == 1) {
		...
		int hyper =
			0;  // hyperthreading status: 0 = unknown, 1 = disabled, 2 = enabled
		if (maxLeaf >=
		    0xB) {  // leaf 0xB or 0x1F: Extended Topology Enumeration
			int num = 0xB;
			...
			for (int c = 0; c < 5; c++) {
				...
				int type =
					(abcd[2] >> 8) & 0xFF;  // enumeration type at level c
				if (type == 1) {            // SMT level
					...
					break;
				// There are more types/levels to consider if we use num = 0x1F.
				// We may need to fix this in the future if CPUs with more
				// complex configurations appear
			}
			physicalProc = logicalProc / procPerCore;

			// The number of performance monitor registers depends on
			// hyperthreading status on Intel CPUs with performance monitoring
			// version 3 or 4
			cpuid(abcd, 0xA, 0);  // performance monitor counters information
			int perfVersion = abcd[0] & 0xFF;  // performance monitoring version
			int perfNum = (abcd[0] >> 8) &
			              0xFF;  // number of performance monitoring registers
			if (perfVersion == 3 || perfVersion == 4) {
				if (perfNum == 4) {
					hyper = 2;  // 4 performance registers when hyperthreading
					            // enabled
				} else if (perfNum == 8) {  // 8 performance registers when
					                        // hyperthreading disabled
					hyper = 1;
					procPerCore = 1;
					logicalProc = physicalProc;  // reduce the number of logical
					                             // processors when
				...
				// hyper remains 0 in all other cases, indicating unknown status
		           ...
		           4) {  // CPUID function 4: cache parameters and cores
			cpuid(abcd, 4);
			logicalProc = (abcd[0] >> 26) + 1;
			if (hyperthreadingSupported) {
				// number of logical processors per core is not known. Assume 2
				// if hyperthreading supported
				procPerCore = 2;
			}
			physicalProc = logicalProc / procPerCore;
		} else {
		...
		if (systemProcessors > logicalProc) {
			// Multiple CPU chips. Assume that chips are identical with respect
			// to hypethreading
		...
		int maxLeaf8 =
			abcd[0] & 0xFFFF;  // maximum eax 0x8000.... input for CPUID
		...
		if (maxLeaf8 >= 8) {
			...
			if (maxLeaf8 >= 0x1E) {
				...
				// procPerCore = 2 if simultaneous multithreading is enabled, 1
				// if disabled
			} else {
				if (hyperthreadingSupported) {
					procPerCore = 2;
			...
			// number of logical processors per core is not known. Assume 2 if
			// SMT supported
			logicalProc = 2;
			physicalProc = 1;
		}
		...
		// The behavior of VIA processors is undocumented! It is not known how
		// to detect threads on a VIA processor
