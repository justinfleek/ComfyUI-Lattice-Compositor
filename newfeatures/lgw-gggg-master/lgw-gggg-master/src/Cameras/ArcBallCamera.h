...
class ArcBallCamera : public ArcBall {
   public:
	...
	void draw(
		const std::vector<
			std::pair<std::reference_wrapper<SceneGraph::Drawable3D>, Matrix4>>&
			drawableTransformations) {
		_camera->draw(drawableTransformations);
	}
	Matrix4 getProjection() { return _camera->projectionMatrix(); }
	Matrix4 getMatrix() { return _camera->cameraMatrix(); }

	void updateCameraProjection() {
		_camera->setProjectionMatrix(Matrix4::perspectiveProjection(
			_fov, Vector2{_windowSize}.aspectRatio(), _near, _far));
		update();
	}
	...
	Float& clipNear() { return _near; }
	Float& clipFar() { return _far; }
