...
void FileSaver::saveToFile(ff::Dynamics::DynamicSystem& dynamics,
...
#pragma omp parallel for
	for (size_t i = 0; i < n_objects; ++i) {
		...
		std::vector<ff::Scalar> timeTransRot;
		timeTransRot.reserve((states.size() + 1) * stateSize);

		ff::Affine transMat = dynamics.renderCollisionGeometry()
		...
		for (const auto& state : states) {
			ff::Affine transform =
				ff::CollisionObject::RigidObjectBase::rigidMatrix(
					state.translation, state.rotation) *
				transMat;
			ff::Affine rotM, scaling;
			transform.computeRotationScaling(&rotM, &scaling);

			ff::Vector3 trans = transform.translation();
			ff::Quaternion rot(rotM.rotation());
