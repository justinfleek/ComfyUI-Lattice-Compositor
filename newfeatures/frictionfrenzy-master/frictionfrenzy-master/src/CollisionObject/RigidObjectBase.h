...
class CPUMeshData {
   public:
	CPUMeshData() {}
	MeshVertexMatrix positions_Eigen;
	std::shared_ptr<MeshIndexMatrix> indices_Eigen;
};
...
class RigidObjectBase {
   public:
	...
	static Isometry rigidMatrix(Eigen::Ref<const Vector3> translate,
		...
		return Eigen::Translation<Scalar, 3>(translate) * quat;
	}
	 ...
	 */
	virtual Scalar obbMinLength() const = 0;
	 ...
	 * @brief Get the pointer to CPU mesh data.
	 */
	void setInputMesh(CPUMeshData* mesh) { mp_mesh = mesh; }
	/**
	 * @brief Get the pointer to CPU mesh data.
	 */
	 ...
	 * @brief Get the pointer to CPU mesh data.
	 */
	void setCollisionMesh(CPUMeshData* mesh) { mp_collisionMesh = mesh; }
	/**
	 ...
	 */
	fcl::CollisionObject<Scalar>* getFCLObject() {
		return mp_collisionObject.get();
	}
	const fcl::CollisionObject<Scalar>* getFCLObject() const {
		return mp_collisionObject.get();
	}
	void setFCLObject(std::unique_ptr<fcl::CollisionObject<Scalar>> obj) {
		mp_collisionObject = std::move(obj);
	}
	/**
	 ...
	 */
	Scalar& density() { return m_density; }
	const Scalar& density() const { return m_density; }
	/**
	...
	/**
	 * @brief Return whether object is sticky
	 */
	bool& getSticky() { return m_isSticky; }
	/**
	 ...
	 *        timestepping.
	 */
	Eigen::Matrix<Scalar, 3, 8> aabbVel() const;
	Matrix3 inertiaTensor() const;
	Matrix3 invInertiaTensor() const;
	const Matrix3& inertiaTensor0() const;
	const Matrix3& invInertiaTensor0() const;
	/**
	 ...
	 * @param[in] pos  the new position.
	 */
	// void setOrigTranslation(Eigen::Ref<const Vector3> pos);
	/**
	...
	/**
	 * @brief Set whether the object is sticky (has adhesion). (deprecated)
	 * @param[in] isSticky  the sticky toggle.
	 */
	void setSticky(bool isSticky) { m_isSticky = isSticky; }
	/**
	 ...
	 */
	void updateDensity(Scalar newDensity);
	/**
	...
	std::shared_ptr<fcl::CollisionGeometry<Scalar>> getCollisionGeometry() {
		return mp_geo;
	}
	void setCollisionGeometry(
		std::shared_ptr<fcl::CollisionGeometry<Scalar>> geo) {
		mp_geo = geo;
	}

	/**
	 * @brief Create a copy of the current object. The configuration is seperate
	 * from the original, while the collision geometry should remain the same.
	 **/
	void copyBaseData(RigidObjectBase* copied) const {
		copied->setStatic(m_isStatic);
		copied->setSticky(m_isSticky);
		copied->updateDensity(m_density);
		copied->setRotation(m_rotation);
		copied->setTranslation(m_translation);
		copied->setCollisionGeometry(mp_geo);
		copied->setFCLObject(std::make_unique<fcl::CollisionObject<Scalar>>(
			mp_geo, copied->getRotation().toRotationMatrix(),
			copied->getTranslation()));
		copied->initializeConfig(getRigidTransMatrix(), m_permanentTrans,
		                         m_origMeshTrans);
	}
	virtual std::unique_ptr<RigidObjectBase> shallowCopy(int id) const = 0;
	/**
	...
	Scalar &squaredDist() {return m_dist2;}
   ...
   protected:
	...
	bool m_isSticky = true;   ///< Is the object sticky?

	/* Mass, density and volume */
	Scalar m_mass,        ///< The mass of the object.
	...
	std::unique_ptr<fcl::CollisionObject<Scalar>>
		mp_collisionObject;  ///< The underlying FCL collision object. (with
	                         ///< ownership)
	std::shared_ptr<fcl::CollisionGeometry<Scalar>>
