...
void RigidObjectBase::updateDensity(Scalar newDensity) {
...
void RigidObjectBase::initializeConfig(const Isometry& rigid,
	...
	mp_collisionObject =
		std::make_unique<fcl::CollisionObject<Scalar>>(mp_geo, rigid);
...
Eigen::Matrix<Scalar, 3, 8> RigidObjectBase::aabbVel() const {
	...
	Eigen::Matrix<Scalar, 3, 8> aabbVertPos;
	for (int i = 0; i < 8; ++i) {
	...
	Eigen::Matrix<Scalar, 3, 8> vertVel;
	vertVel.colwise() = m_velocity;
	vertVel -= aabbVertPos.colwise().cross(m_angularVel);

	return vertVel;
}
Matrix3 RigidObjectBase::inertiaTensor() const {
	Matrix3 rot = m_rotation.toRotationMatrix();
	return rot * m_moment0 * rot.transpose();
}
Matrix3 RigidObjectBase::invInertiaTensor() const {
	Matrix3 rot = m_rotation.toRotationMatrix();
	return rot * m_iMoment0 * rot.transpose();
}
const Matrix3& RigidObjectBase::inertiaTensor0() const {
	return m_moment0;
}
const Matrix3& RigidObjectBase::invInertiaTensor0() const {
	return m_iMoment0;
}
...
void RigidObjectBase::stripTransformation(
	...
	volume = 0;
	MeshIndexMatrix& indices = *(mesh.indices_Eigen);
	for (Eigen::Index i = 0; i < indices.rows(); ++i) {
		...
		Scalar volTet = v1.cross(v2).dot(v3) / 6;
