...
void Ellipsoid::initializeFCLObject() {
	...
	mp_geo = std::make_unique<fcl::Ellipsoid<Scalar>>(m_dimensions);
	...
	m_dist2 =
		(4.0 / 15) * M_PI * m_dimensions.prod() * m_dimensions.squaredNorm();

	initializeConfig(rigid, scaled, meshScaled);
}
Scalar Ellipsoid::obbMinLength() const {
	// Minimum length of the OBB is the minimum length of the ellipsoid
	auto* ellipsoid = dynamic_cast<fcl::Ellipsoid<Scalar>*>(mp_geo.get());
