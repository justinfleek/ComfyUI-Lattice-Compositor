...
class ContactInformation {
   public:
	typedef Eigen::Matrix<Scalar, 6, 3> HalfDelassus;
	 ...
	 */
	Scalar toNormal(const Eigen::Ref<const VectorX>& globalQuants) const {
	 ...
	 * @returns The 6-dim global quantity
	 */
	Vector6 toGlobalNormal(bool T, const Scalar& localQuant) const {
	 ...
	 * @returns The constraint jacobian.
	 */
	Eigen::Matrix<Scalar, 6, 2> getTransTangent(bool T) const {
		return -transformation[T].block<6, 2>(0, 1);
	}

	/**
	 * Calculate the (virtual) displacement of the contact point from object
	 * COM, in world coordinates.
	 */
	Vector3 displacement(bool T) const {
		Matrix3 crossMat = transformation[T].block<3, 3>(3, 0) *
		                   transformation[T].block<3, 3>(0, 0).transpose();
		Vector3 disp(0.5 * crossMat(2, 1) - 0.5 * crossMat(1, 2),
	...
	std::string toString() const {
		...
		ss << "Position: " << pos.transpose()
		   ...
		   << "\nDynamic force: " << dynamicFriction.transpose();
		return ss.str();
	}
	...
	Vector2 dynamicFriction;  /// The dinamic friction (obsolete).
	bool isDynamic;   /// Whether the current contact has dynamic friction
	                  /// (obsolete).
	Scalar depth;     /// Penetration depth.
	Scalar area;      /// Contact area.
