...
void MeshMeshContactGenerator::operator()(
	...
	struct ContactWithArea {
		fcl::Contact<Scalar> c;
		Scalar area;
		int qObj;
	};
	...
	auto pushVertexContact = [globalVertPosition, &meshPtr0, &meshPtr1,
		...
		if (!visited.count(vert)) {
			...
			const auto sdfq = queryPtr->sdfQuery().lock();

			Vector3 vertNormal = queryPtr->getRotation() *
			                     sdfq->vertNormals.row(vert).transpose();

			if (dist < 0) {
				if (vertNormal.dot(n) < 0) {
					fcl::Contact<Scalar> c;
					int sign = qObj ? 1 : -1;
					...
					toPush.push_back(
						{c, sdfq->vertArea[vert] * -vertNormal.dot(n), qObj});
				}
				q.push(vert);
	...
	for (int qObj = 0; qObj < 2; ++qObj) {
		while (!vertQ[qObj].empty()) {
			...
			vertQ[qObj].pop();
			const auto& adjMap =
				meshPtrs[qObj][0]->sdfQuery().lock()->vertAdjMap;
	...
	for (size_t i = 0; i < toPush.size(); ++i) {
		out.push_back(
			Contact::convertContact(toPush[i].c, toPush[i].area, pObj0, pObj1));
	}
}
}  // namespace Contact
}  // namespace FrictionFrenzy
