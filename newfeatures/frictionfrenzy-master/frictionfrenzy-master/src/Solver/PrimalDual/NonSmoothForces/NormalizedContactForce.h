#pragma once
#include "ForceConstraint/CylinderConstraint.h"
#include "NonSmoothForceBase.h"

namespace FrictionFrenzy {
namespace Params {
struct NormalizedContactForce : public NonSmoothForce {
...
namespace Solver {
class NormalizedContactForce : public NonSmoothForceBase {
   public:
	NormalizedContactForce(std::shared_ptr<unsigned int> logging)
		: NonSmoothForceBase(logging) {
		mp_frictionCoeff = std::make_shared<Scalar>(0.5);
		mp_constraint = std::make_unique<CylinderConstraint>(mp_frictionCoeff);
	}
	...
	NonSmoothForceType getType() const override {
		return NonSmoothForceType::NON_SMOOTH_CONTACTS_NORMALIZED;
	}
