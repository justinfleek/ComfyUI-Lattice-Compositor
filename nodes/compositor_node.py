            ...
            if key == 'expression' and isinstance(value, str):
                ...
                for expr_key, expr_value in value.items():
                    if isinstance(expr_value, str):
                        expr_errors = _validate_single_expression(
                            expr_value, f"{current_path}.{expr_key}"
                        )
                        errors.extend(expr_errors)
            else:
                # Recurse
...
def _validate_single_expression(expr: str, path: str) -> List[str]:
    ...
    for pattern in COMPILED_DANGEROUS_PATTERNS:
        if pattern.search(expr):
            ...
            security_logger.warning(f"SECURITY: Blocked expression with pattern {pattern.pattern} at {path}")
            ...
            if isinstance(value, (int, float)):
                ...
                if key in NUMERIC_BOUNDS:
                    ...
                    if not (min_val <= value <= max_val):
                        errors.append(
                            f"Value out of range at {current_path}: {value} "
...
try:
    ...
    async def save_project(request):
        ...
        try:
            ...
            if content_length > MAX_PROJECT_SIZE_BYTES:
                security_logger.warning(f"SECURITY: Oversized project upload attempted: {content_length} bytes")
            ...
            try:
                ...
                validated_project, errors, warnings = validate_project(project_data, len(project_json))
    ...
    async def load_project(request):
        ...
        try:
            ...
            try:
                validated_project, errors, warnings = validate_project(project, file_size)
            except ProjectValidationError as e:
