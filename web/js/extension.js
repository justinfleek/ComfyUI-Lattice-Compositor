...
function handleInputsReady(event) {
  ...
  if (state.vueAppLoaded) {
    window.dispatchEvent(
      new CustomEvent("lattice:inputs-ready", { detail: event.detail }),
    );
  } else {
...
app.registerExtension({
  ...
  async setup() {
    ...
    if (app.extensionManager?.registerSidebarTab) {
      ...
      console.warn(
        "[Lattice] extensionManager.registerSidebarTab not available - ComfyUI version may be too old",
      );
      console.warn(
        "[Lattice] Try updating ComfyUI to the latest version with the new frontend",
      );
    }
...
async function renderCompositor(el, base) {
  ...
  container.style.cssText =
    "width:100%;height:100%;min-height:100vh;overflow:hidden;background:#050505;position:relative;";
  ...
  try {
    ...
    state.pendingMessages.forEach((data) => {
      window.dispatchEvent(
        new CustomEvent("lattice:inputs-ready", { detail: data }),
      );
    });
