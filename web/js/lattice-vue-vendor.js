...
function trigger(target, type, key, newValue, _oldValue, _oldTarget) {
...
function computed$1(getterOrOptions, _debugOptions, isSSR = false) {
  let getter;
  let setter;
...
function flushPreFlushCbs(instance, _seen, i = flushIndex + 1) {
...
function flushPostFlushCbs(_seen) {
...
function renderList(source, renderItem, cache, _index) {
  let ret;
  const cached = cache;
...
function getContext(_calledFunctionName) {
  const i = getCurrentInstance();
...
function resolveInjections(
  injectOptions,
  ctx,
  _checkDuplicateProperties = NOOP,
) {
...
let uid$1 = 0;
function createAppAPI(render, _hydrate) {
  return function createApp(rootComponent, rootProps = null) {
    ...
    const app = (context.app = {
      ...
      mount(rootContainer, _isHydrate, namespace) {
        if (!isMounted) {
...
const normalizeSlot = (_key, rawSlot, ctx) => {
  if (rawSlot._n) {
    return rawSlot;
  }
...
const normalizeObjectSlots = (rawSlots, slots, _instance) => {
...
function baseCreateRenderer(options, _createHydrationFns) {
  ...
  const patch = (
    ...
    if (ref3 != null && parentComponent) {
      setRef(ref3, n1?.ref, parentSuspense, n2 || n1, !n2);
