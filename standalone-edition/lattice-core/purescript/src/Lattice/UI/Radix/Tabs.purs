...
-- | Provides behavioral accessibility features:
-- |
...
-- |
-- | This is a SLOT-BASED component - parents provide content via the `panels` record:
-- | Each tab value maps to a ComponentHTML that renders when that tab is selected.
-- |
-- | STYLING: Uses Lattice design tokens from design-tokens.css
module Lattice.UI.Radix.Tabs
  ( component
  , Query(..)
  , Input
  , Output(..)
...
import Web.DOM.NonElementParentNode (getElementById) as NEPN
...
-- | The component accepts a record mapping tab values to content HTML
-- | This allows parents to provide custom content for each tab panel
component :: forall m. MonadAff m 
  => Object (H.ComponentHTML Action () m)  -- Map of tab value -> panel content
  -> H.Component Query Input Output m
component panels = H.mkComponent
  { initialState
  , render: renderWithPanels panels
...
renderWithPanels :: forall m. MonadAff m 
  => Object (H.ComponentHTML Action () m)
  -> State 
  -> H.ComponentHTML Action () m
renderWithPanels panels state =
  HH.div
    [ HP.class_ (HH.ClassName "lattice-tabs-root")
    , HP.attr (HH.AttrName "data-orientation") (orientationStr state.input.orientation)
    ]
    [ renderTabList state
    , renderTabPanels panels state
    ]

renderTabList :: forall m. State -> H.ComponentHTML Action () m
renderTabList state =
  HH.div
    [ HP.class_ (HH.ClassName "lattice-tabs-list")
...
renderTab state idx tab =
  ...
  in
  HH.button
    [ HP.class_ (HH.ClassName "lattice-tabs-trigger")
...
renderTabPanels :: forall m. MonadAff m 
  => Object (H.ComponentHTML Action () m)
  -> State 
  -> H.ComponentHTML Action () m
renderTabPanels panels state =
  HH.div
    [ HP.class_ (HH.ClassName "lattice-tabs-panels") ]
    (map (renderTabPanel panels state) state.input.tabs)

renderTabPanel :: forall m. MonadAff m 
  => Object (H.ComponentHTML Action () m)
  -> State 
  -> Tab 
  -> H.ComponentHTML Action () m
renderTabPanel panels state tab =
  let
    isSelected = tab.value == state.selectedValue
    tabId = state.baseId <> "-tab-" <> tab.value
    panelId = state.baseId <> "-panel-" <> tab.value
    panelContent = fromMaybe (HH.text "") (Object.lookup tab.value panels)
  in
  if isSelected then
    HH.div
      [ HP.class_ (HH.ClassName "lattice-tabs-content")
      , HP.id panelId
...
-- | Get element by ID using web-dom NonElementParentNode API
getElementById :: String -> HTMLDocument.HTMLDocument -> Effect (Maybe HTMLElement.HTMLElement)
getElementById id doc = do
  mElement <- NEPN.getElementById id (HTMLDocument.toNonElementParentNode doc)
  pure $ mElement >>= HTMLElement.fromElement
