...
{- | A relative path within an output.
...
For single-output packages, use 'outPath' which creates paths in $out.
For multiple-output packages, use 'outPathNamed' to specify the output name.

Cannot be constructed from arbitrary Text - use smart constructors.
-}
data OutPath
    = -- | Path in the default output ($out)
      OutPathDefault !Text
    | -- | Path in a named output ($dev/include, $lib/lib)
      OutPathNamed !Text !Text
    deriving stock (Eq, Ord, Show, Generic)
...
For cross-compilation, use @BuildPkgRef@ to reference build-time packages:
@BuildPkgRef "cmake"@ becomes @${pkgs.buildPackages.cmake}@ in Nix.
-}
data PkgRef
    = -- | Reference a regular package: "openssl" → ${pkgs.openssl}
      PkgName !Text
    | -- | Reference a build-time package: "cmake" → ${pkgs.buildPackages.cmake}
      BuildPkgRef !Text
    deriving stock (Eq, Ord, Show, Generic)
...
-- | Path within a named output (e.g., $dev/include, $lib/lib)
-- Example: outPathNamed "dev" "include/foo.h" → $dev/include/foo.h
outPathNamed :: Text -> Text -> OutPath
...
-- | Reference to a build-time package (for cross-compilation)
-- Example: buildPkgRef "cmake" → ${pkgs.buildPackages.cmake}
buildPkgRef :: Text -> PkgRef
