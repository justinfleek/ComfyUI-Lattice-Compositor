...
std::string greet(const std::string& name) {
    return "Hello from straylight, " + name + "!";
}
...
NB_MODULE(example, m) {
    m.doc() = "straylight example module: C++ called from Python via nanobind";

    // Simple functions
    m.def("add", &straylight::add, "Add two integers",
          nb::arg("a"), nb::arg("b"));

    m.def("greet", &straylight::greet, "Generate a greeting",
          nb::arg("name"));

    // Vector functions
    m.def("dot_product", &straylight::dot_product,
          "Compute dot product of two vectors",
          nb::arg("a"), nb::arg("b"));

    m.def("norm", &straylight::norm,
          "Compute Euclidean norm of a vector",
          nb::arg("v"));

    // Class binding
    nb::class_<straylight::Counter>(m, "Counter")
        .def(nb::init<int>(), nb::arg("initial") = 0)
        .def("increment", &straylight::Counter::increment)
        .def("decrement", &straylight::Counter::decrement)
        .def("add", &straylight::Counter::add, nb::arg("n"))
        .def("get", &straylight::Counter::get)
        .def("reset", &straylight::Counter::reset)
        .def("__repr__", &straylight::Counter::repr);
    ...
    m.def("test", []() {
        ...
        if (std::abs(straylight::dot_product(a, b) - expected) > 1e-10) {
            ok = false;
        }

        // Test Counter
        straylight::Counter c(10);
        c.increment();
        if (c.get() != 11) {
            ok = false;
        }
