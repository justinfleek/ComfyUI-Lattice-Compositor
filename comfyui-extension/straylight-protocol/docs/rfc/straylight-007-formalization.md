...
Located in `nix/scripts/Straylight/Nix/`:
...
```haskell
module Straylight.Nix.Packages.Nvidia where
...
| `Straylight.Nix.{FFI,Value,Derivation,Syntax}` | **Complete** |
...
| 7 | Incremental adoption (`straylight.phases.interpret`) | **Complete** |
...
### Phase 7: Incremental Adoption ✅ **Complete**
...
**Implementation**: `nix/prelude/phases.nix`

**Usage**: See `nix/packages/test-interpret.nix` for example.
...
final: prev: {
  writeShellApplication = throw "Use a .hs file with Straylight.Script";
  writeShellScript = throw "Use a .hs file with Straylight.Script";
...
Typed tool modules live in `nix/scripts/Straylight/Nix/Tools/`. Each provides a
type-safe interface to a common build tool, with automatic dependency tracking.

### Jq (`Straylight.Nix.Tools.Jq`)
...
### PatchElf (`Straylight.Nix.Tools.PatchElf`)
...
### Install (`Straylight.Nix.Tools.Install`)
...
### Substitute (`Straylight.Nix.Tools.Substitute`)
...
1. Create `nix/scripts/Straylight/Nix/Tools/MyTool.hs`
...
1. Re-export from `Straylight.Nix.Tools`
