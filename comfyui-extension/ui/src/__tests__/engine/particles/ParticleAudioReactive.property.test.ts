...
> {
  const emitter = {
    ...
    colorStart: [1, 1, 1, 1] as [number, number, number, number],
    colorEnd: [1, 1, 1, 0] as [number, number, number, number],
    colorVariance: 0,
...
describe("ParticleAudioReactive.getModulation", () => {
  ...
  it("should return Err for unbound parameter", () => {
    const binding = createDefaultBinding();
    system.setBindings([binding]);

    // queryModulation returns Err (not found) for unbound parameter
    const result = system.queryModulation("emitter", "emitter-1", "nonexistent");
