...
describe('Emitter Management', () => {
  ...
  test.prop([
    ...
    // Before removal: queryEmitter returns Ok
    const beforeResult = system.queryEmitter(id);
    expect(beforeResult.ok).toBe(true);
    
    system.removeEmitter(id);
    
    // After removal: queryEmitter returns Err (not found)
    const afterResult = system.queryEmitter(id);
    expect(afterResult.ok).toBe(false);
    if (!afterResult.ok) {
      expect(afterResult.error.kind).toBe("not_found");
    }
