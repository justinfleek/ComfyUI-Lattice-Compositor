...
// ============================================================================
// Test Helper: createTestKeyframe
// ============================================================================
// The production createKeyframe now requires (layerId, propertyPath, frame, value)
// for deterministic ID generation. This helper provides the old (frame, value) API
// for tests that don't need specific layer/property context.

import { createKeyframe as prodCreateKeyframe } from "@/types/animation";
import type { Keyframe, InterpolationType } from "@/types/animation";

/**
 * Test helper that wraps createKeyframe with default layerId and propertyPath.
 * Use this in tests where deterministic IDs don't matter.
 * 
...
export function createTestKeyframe<T>(
  ...
  return prodCreateKeyframe("test-layer", "test.property", frame, value, interpolation);
}

// Make it globally available for tests
const globalWithTestHelpers = globalThis as typeof globalThis & { 
  createTestKeyframe: typeof createTestKeyframe;
};
globalWithTestHelpers.createTestKeyframe = createTestKeyframe;
