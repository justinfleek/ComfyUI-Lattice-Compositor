...
describe('createKeyframe', () => {
  ...
  describe('basic functionality', () => {
    test('creates keyframe with required fields', () => {
      const kf = createKeyframe('test-layer', 'test.property', 0, 100);
    ...
    test('creates keyframe with all parameters', () => {
      const kf = createKeyframe('test-layer', 'test.property', 30, 50, 'bezier');
    ...
    test('id starts with kf_ prefix', () => {
      const kf = createKeyframe('test-layer', 'test.property', 0, 0);
      expect(kf.id.startsWith('kf_')).toBe(true);
    });

    test('id contains frame number', () => {
      const kf = createKeyframe('test-layer', 'test.property', 42, 0);
  ...
  describe('interpolation types', () => {
    ...
    test('interpolation: linear (explicit)', () => {
      const kf = createKeyframe('test-layer', 'test.property', 0, 0, 'linear');
      expect(kf.interpolation).toBe('linear');
    });

    test('interpolation: bezier', () => {
      const kf = createKeyframe('test-layer', 'test.property', 0, 0, 'bezier');
      expect(kf.interpolation).toBe('bezier');
    });

    test('interpolation: hold', () => {
      const kf = createKeyframe('test-layer', 'test.property', 0, 0, 'hold');
    ...
    test.each(easingTypes)('interpolation: %s', (easing) => {
      const kf = createKeyframe('test-layer', 'test.property', 0, 0, easing);
