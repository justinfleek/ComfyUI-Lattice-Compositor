...
describe('PROPERTY: getLayerData', () => {
  ...
  test.prop([layerTypeArb, layerTypeArb])('returns Err when types differ', (type1, type2) => {
    fc.pre(type1 !== type2);
    const layer = createMinimalLayer(type1);
    // Set test data matching layer type
    const testData = { testField: 'value' };
    layer.data = testData as typeof layer.data;
    // queryLayerData returns Err (type mismatch) instead of null
    const result = queryLayerData(layer, type2);
  ...
  test.prop([layerArb])('queryLayerData never throws', (layer) => {
    for (const type of ALL_LAYER_TYPES) {
      // queryLayerData is a total function - never throws
      expect(() => queryLayerData(layer, type)).not.toThrow();
    }
  });
