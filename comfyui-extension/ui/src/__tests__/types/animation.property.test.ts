...
describe("PROPERTY: createKeyframe", () => {
  it("returns object with all required fields", () => {
    fc.assert(
      fc.property(validFrameArb, numberValueArb, interpolationTypeArb, (frame, value, interp) => {
        const result = createKeyframe("test-layer", "test.property", frame, value, interp);
  ...
  it("id starts with 'kf_'", () => {
    fc.assert(
      fc.property(validFrameArb, numberValueArb, (frame, value) => {
        const result = createKeyframe("test-layer", "test.property", frame, value);
  ...
  it("consecutive calls produce unique IDs", () => {
    fc.assert(
      fc.property(validFrameArb, numberValueArb, (frame, value) => {
        const result1 = createKeyframe("test-layer", "test.property", frame, value);
        const result2 = createKeyframe("test-layer", "test.property", frame, value);
        expect(result1.id).not.toBe(result2.id);
      })
    );
  });
  ...
  it("throws on NaN frame (BUG-002 fix verification)", () => {
    expect(() => createKeyframe("test-layer", "test.property", NaN, 100)).toThrow("Invalid keyframe frame");
  });

  it("throws on Infinity frame (BUG-002 fix verification)", () => {
    expect(() => createKeyframe("test-layer", "test.property", Infinity, 100)).toThrow("Invalid keyframe frame");
  });
  ...
  it("accepts zero as valid frame", () => {
    const result = createKeyframe("test-layer", "test.property", 0, 100);
  ...
  it("handles all 33 interpolation types", () => {
    ...
    for (const interpType of allTypes) {
      const result = createKeyframe("test-layer", "test.property", 0, 100, interpType);
