...
import Lattice.Utils.NumericSafety (ensureFiniteD, clampD)
...
processAudioValue audioValue mapping =
  let -- Apply threshold (noise gate)
      ...
      afterCurve = if mappingAmplitudeCurve mapping /= 1.0
                   then let curve = ensureFiniteD (mappingAmplitudeCurve mapping) 1.0
      ...
      clamped = clampD afterInvert (mappingMin mapping) (mappingMax mapping)
  in ensureFiniteD clamped 0.0
