...
rotatePoint2D (Point2D x y) angle =
  ...
  in Point2D (ensureFiniteD newX 0.0) (ensureFiniteD newY 0.0)
...
cubicBezierDerivative2D p0 p1 p2 p3 t =
  ...
  in Point2D (ensureFiniteD dx 0.0) (ensureFiniteD dy 0.0)
...
cubicBezierLength2D p0 p1 p2 p3 subdivisions =
  let subdivisionsD = fromIntegral subdivisions
      ...
      bezierAt t =
        ...
        in Point2D (ensureFiniteD x 0.0) (ensureFiniteD y 0.0)
      -- Helper to calculate distance
      pointDistance (Point2D x1 y1) (Point2D x2 y2) =
        let dx = x2 - x1
            dy = y2 - y1
        in safeSqrtD (dx * dx + dy * dy)
