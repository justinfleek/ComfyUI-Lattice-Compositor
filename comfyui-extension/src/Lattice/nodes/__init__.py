...
- lattice_video_generation.py: 4 routes (MonarchRT video generation)
...
def register_all_routes() -> dict:
  ...
  route_modules = [
    ...
    ("lattice_video_generation", 4),  # MonarchRT video generation
  ]

  # Total includes compositor_node's 11 routes
  total_expected = 11 + sum(count for _, count in route_modules)  # 51
