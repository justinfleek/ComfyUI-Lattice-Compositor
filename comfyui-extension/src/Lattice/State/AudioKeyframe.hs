...
applySmoothing values factor =
  let
    safeFactor = ensureFiniteD factor 0.0
  ...
  in
    case values of
      ...
      (first : rest) ->
        let
          safeFirst = ensureFiniteD first 0.0
          smoothed = scanl smoothNext safeFirst rest
          smoothNext prev curr =
            let
              safeCurr = ensureFiniteD curr 0.0
            in
...
interpolateKeyframeValue targetFrame keyframes defaultValue =
  ...
  in
    case exactMatch of
      Just (_, value) -> ensureFiniteD value defaultValue
      Nothing ->
        let
        ...
        in
          case (previousKf, nextKf) of
            ...
            (Nothing, Just (_, nextValue)) -> ensureFiniteD nextValue defaultValue
            (Just (_, prevValue), Nothing) -> ensureFiniteD prevValue defaultValue
            (Just (prevFrame, prevValue), Just (nextFrame, nextValue)) ->
              let
                safePrevValue = ensureFiniteD prevValue defaultValue
                safeNextValue = ensureFiniteD nextValue defaultValue
