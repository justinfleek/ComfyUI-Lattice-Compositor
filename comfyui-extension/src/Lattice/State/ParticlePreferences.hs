...
import Lattice.Utils.NumericSafety (ensureFiniteD, isFinite)
...
sanitizeMaxParticlesPerLayer mVal =
  case mVal of
    ...
    Just val ->
      ...
      else ensureFiniteD (fromIntegral (max 1000 (min 500000 (floor val)))) 100000.0
...
sanitizeCacheCheckpointInterval mVal =
  case mVal of
    ...
    Just val ->
      if not (isFinite val) || val <= 0
      then 30.0 -- default
      else ensureFiniteD (fromIntegral (max 1 (min 120 (floor val)))) 30.0
...
sanitizeMaxCacheMemoryMB mVal =
  case mVal of
    ...
    Just val ->
      if not (isFinite val) || val <= 0
      then 512.0 -- default
      else ensureFiniteD (fromIntegral (max 128 (min 2048 (floor val)))) 512.0
