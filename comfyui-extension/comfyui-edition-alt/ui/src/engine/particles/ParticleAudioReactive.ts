...
export class ParticleAudioReactive {
  ...
  /**
   * Get audio modulation for a specific parameter (strict version - throws if not found)
   *
   ...
   * @see queryModulation for a Result-based version with full error context
   */
  getModulation(target: string, targetId: string, parameter: string): number {
    const result = this.queryModulation(target, targetId, parameter);
    if (result.ok) {
      return result.value;
    }
    throw new Error(
      `[ParticleAudioReactive] Cannot get modulation: ${result.error.message} ` +
        `Use queryModulation() for Result-based error handling.`,
    );
  }

  /**
   * Query audio modulation for a specific parameter with Result-based error handling
   *
   * System F/Omega proof: Total function - always returns, never throws
   * Type proof: (target, targetId, parameter) → Result<number, NotFoundError>
   * Mathematical proof: Exhaustive pattern matching on Result required
   *
   * @returns Result.Ok with modulation value if binding found, Result.Err with NotFoundError otherwise
   */
  queryModulation(
    target: string,
    targetId: string,
    parameter: string,
  ): Result<number, NotFoundError> {
    for (const binding of this.audioBindings) {
      if (
        ...
        const safeOutputMin = Number.isFinite(binding.outputMin)
          ? binding.outputMin
          : 0;
        const safeOutputMax = Number.isFinite(binding.outputMax)
          ? binding.outputMax
          : 1;
        return ok(safeOutputMin + t * (safeOutputMax - safeOutputMin));
      }
    }

    // Build list of available bindings for error context
    const availableBindings = this.audioBindings.map(
      (b) => `${b.target}/${b.targetId}/${b.parameter}`,
    );

    return err(
      notFoundError(
        "AudioBinding",
        `${target}/${targetId}/${parameter}`,
        availableBindings,
      ),
    );
  }
  ...
  applyModulation(
    ...
    for (let i = 0; i < this.audioBindings.length; i++) {
      ...
      const previousSmoothed =
        previousSmoothedRaw !== null &&
        previousSmoothedRaw !== undefined &&
        typeof previousSmoothedRaw === "number" &&
        Number.isFinite(previousSmoothedRaw)
          ? previousSmoothedRaw
          : featureValue;
      // Validate smoothing
      const safeSmoothing = Number.isFinite(binding.smoothing)
        ? Math.max(0, Math.min(1, binding.smoothing))
        : 0;
      ...
      // Map to output range
      const t = Math.max(0, Math.min(1, (smoothed - safeMin) / safeRange));
      ...
      if (binding.curve === "exponential") {
        ...
        // Lean4/PureScript/Haskell: Explicit pattern matching - no lazy ??
        const stepCount =
          typeof binding.stepCount === "number" &&
          Number.isFinite(binding.stepCount) &&
          binding.stepCount >= 2
            ? binding.stepCount
            : 5;
      ...
      // Lean4/PureScript/Haskell: Explicit pattern matching - no lazy ??
      const triggerMode =
        typeof binding.triggerMode === "string" &&
        binding.triggerMode.length > 0
          ? binding.triggerMode
          : "continuous";
      if (triggerMode === "onThreshold") {
        ...
        const onsetValue =
          onsetValueRaw !== null &&
          onsetValueRaw !== undefined &&
          typeof onsetValueRaw === "number" &&
          Number.isFinite(onsetValueRaw)
            ? onsetValueRaw
            : 0;
      ...
      if (binding.target === "emitter") {
        ...
        if (emitter) {
          ...
          if (
            param === "emissionRate" ||
            param === "emissionRateVariance" ||
            param === "burstCount" ||
            param === "burstInterval" ||
            param === "initialSpeed" ||
            param === "speedVariance" ||
            param === "inheritEmitterVelocity" ||
            param === "initialSize" ||
            param === "sizeVariance" ||
            param === "initialMass" ||
            param === "massVariance" ||
            param === "lifetime" ||
            param === "lifetimeVariance" ||
            param === "initialRotation" ||
            param === "rotationVariance" ||
            param === "initialAngularVelocity" ||
            param === "angularVelocityVariance" ||
            param === "colorVariance" ||
            param === "emissionSpread" ||
            param === "beatEmissionMultiplier"
          ) {
        ...
        if (field) {
          ...
          if (
            ...
            param === "quadraticDrag" ||
            param === "bounceDamping" ||
            param === "collisionRestitution" ||
            param === "collisionFriction" ||
            param === "pathStrength" ||
            param === "pathRadius" ||
            param === "lorenzSigma" ||
            param === "lorenzRho" ||
