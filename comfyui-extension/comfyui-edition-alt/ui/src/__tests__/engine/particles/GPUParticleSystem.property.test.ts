...
describe("VerifiedGPUParticleSystem Buffer Layout", () => {
  ...
  it("buffer layout matches type definition", () => {
    ...
    expect(Math.max(...Object.values(expectedLayout)) + 1).toBe(
      PARTICLE_STRIDE,
    );
  });
});
...
describe("VerifiedGPUParticleSystem Configuration Validation", () => {
  test.prop([fc.integer({ min: -1000000, max: 2000000 })])(
    "maxParticles is capped and validated",
    (maxParticles) => {
  ...
  test.prop([fc.integer({ min: 0, max: 1000000 })])(
    "randomSeed produces deterministic RNG",
    (seed) => {
      const system1 = createTestableSystem({
        randomSeed: seed,
        maxParticles: 100,
      });
      const system2 = createTestableSystem({
        randomSeed: seed,
        maxParticles: 100,
      });
...
describe("Default Factory Functions", () => {
  test.prop([
    fc.constantFrom(
      ...
      "gravity" | "vortex" | "turbulence" | "drag" | "wind" | "lorenz"
    >,
...
describe("Emitter Management", () => {
  ...
  test.prop([fc.string({ minLength: 1, maxLength: 50 })])(
    "addEmitter and getEmitter round-trip",
    (id) => {
  ...
  test.prop([fc.string({ minLength: 1, maxLength: 50 })])(
    "removeEmitter removes the emitter",
    (id) => {
      const emitter = createDefaultEmitter(id);
      system.addEmitter(emitter);
      ...
      // After removal, getEmitter throws (not returns undefined)
      expect(() => system.getEmitter(id)).toThrow(/Emitter not found/);
...
describe("Simulation", () => {
  ...
  test.prop([fc.integer({ min: 0, max: 1000000 })])(
    "reset restores initial state with same seed",
    (seed) => {
      const system = createTestableSystem({
        randomSeed: seed,
        maxParticles: 100,
      });
...
describe("Particle Emission Validation", () => {
  ...
  test.prop([
    ...
    "emitter validates mass variance (BUG-087 regression)",
    (mass, variance) => {
      const system = createTestableSystem({ maxParticles: 100 });
  ...
  test.prop([
    ...
    "emitter validates size variance (BUG-089 regression)",
    (size, variance) => {
      const system = createTestableSystem({ maxParticles: 100 });
...
describe("RNG Determinism", () => {
  test.prop([
    ...
    "same seed produces identical particle counts after N steps",
    (seed, steps) => {
      ...
      expect(system1.getState().particleCount).toBe(
        system2.getState().particleCount,
      );
  ...
  test.prop([
    ...
    "different seeds produce different results (except collisions)",
    (seed1, seed2) => {
      fc.pre(seed1 !== seed2);
