 ...
 */
const _OFFICIAL_SIGNED_TEMPLATE = {
  name: "Test Template",
  version: "1.0.0",
  layers: [
 ...
 */
const _THIRD_PARTY_SIGNED_TEMPLATE = {
...
const _INVALID_SIGNATURE_TEMPLATE = {
...
const TEST_PRIVATE_KEY =
  "gt0SdUQuTv6XFIR61+wMQ2BKDacdeNHRaEyw+k9yI0TGYJZd9ErDsNAu7JB2pAgmUBv6vRqulS7ahWNN0mC/+Q==";
...
describe("Template Signature Verification", () => {
  describe("verifyTemplate", () => {
    it("should verify an officially signed template", async () => {
      ...
      const templateData = {
        ...
        version: "1.0.0",
        layers: [{ id: "bg", name: "Background", type: "solid" }],
      };

      const signedTemplate = await createSignedTemplate(
        templateData,
        TEST_PRIVATE_KEY,
      );
  ...
  describe("loadAndVerifyTemplate", () => {
    ...
    it("should handle invalid JSON gracefully", async () => {
      const { template, verification } =
        await loadAndVerifyTemplate("not valid json");
  ...
  describe("Canonical JSON ordering", () => {
    it("should produce same signature regardless of property order", async () => {
      ...
      expect((signed1._signature as { signature: string }).signature).toBe(
        (signed2._signature as { signature: string }).signature,
      );
    });
  });
});
