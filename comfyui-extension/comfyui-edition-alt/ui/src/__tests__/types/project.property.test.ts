...
const positiveDimensionArb = fc
  .integer({ min: 1, max: 1000 })
  .map((n) => n * 8);

// Invalid dimensions (NOT divisible by 8)
const invalidDimensionArb = fc
  .integer({ min: 1, max: 7999 })
  .filter((n) => n % 8 !== 0);
...
describe("PROPERTY: isLayerOfType", () => {
  ...
  test.prop([layerTypeArb, layerTypeArb])(
    "returns false when types differ",
    (type1, type2) => {
...
describe("PROPERTY: getLayerData", () => {
  ...
  test.prop([layerTypeArb, layerTypeArb])(
    ...
    (type1, type2) => {
      ...
      const testData = { testField: "value" };
      layer.data = testData as typeof layer.data;
      // getLayerData throws on type mismatch (not returns null)
      expect(() => getLayerData(layer, type2)).toThrow(/Layer type mismatch/);
    },
  );

  test.prop([layerArb])(
    "throws on type mismatch, returns data on match",
    (layer) => {
      for (const type of ALL_LAYER_TYPES) {
        if (type === layer.type) {
          ...
          // Should throw when types differ
          expect(() => getLayerData(layer, type)).toThrow(
            /Layer type mismatch/,
          );
        }
      }
...
describe("PROPERTY: createDefaultEffectLayerData", () => {
  ...
  test.prop([fc.integer({ min: 1, max: 100 })])(
    "is deterministic (same output every call)",
    (n) => {
...
describe("PROPERTY: createDefaultPoseLayerData", () => {
  test.prop([fc.constant(null)])(
    "always returns exactly 18 keypoints (COCO18)",
    () => {
  ...
  test.prop([fc.integer({ min: 1, max: 100 })])("is deterministic", (n) => {
    ...
    for (let i = 1; i < results.length; i++) {
      ...
      expect(results[i].poses[0].keypoints.length).toBe(
        results[0].poses[0].keypoints.length,
      );
    }
  });
});
...
describe("PROPERTY: createDefaultProceduralMatteData", () => {
  ...
  test.prop([patternTypeArb])(
    "parameters has expected structure",
    (patternType) => {
      const data = createDefaultProceduralMatteData(patternType);
  ...
  test.prop([patternTypeArb])(
    "animation has expected structure",
    (patternType) => {
      const data = createDefaultProceduralMatteData(patternType);
...
describe("PROPERTY: normalizeLayerTiming", () => {
  ...
  test.prop([frameArb, frameArb])(
    "sets inPoint/outPoint from startFrame/endFrame",
    (start, end) => {
      const layer = createMinimalLayer("solid");
      layer.startFrame = start;
      layer.endFrame = end;
  ...
  test.prop([frameArb, frameArb])(
    "migrates inPoint/outPoint to startFrame/endFrame",
    (inP, outP) => {
      ...
      const layer: Omit<Layer, "startFrame" | "endFrame"> & {
        inPoint: number;
        outPoint: number;
      } = {
        ...layerWithoutFrames,
...
describe("PROPERTY: createEmptyProject", () => {
  ...
  test.prop([positiveDimensionArb, positiveDimensionArb])(
    ...
    (width, height) => {
      ...
      expect(project.composition.width).toBe(
        project.compositions.main.settings.width,
      );
      expect(project.composition.height).toBe(
        project.compositions.main.settings.height,
      );
      expect(project.composition.fps).toBe(
        project.compositions.main.settings.fps,
      );
    },
  );
...
describe("PROPERTY: BLEND_MODE_CATEGORIES", () => {
  ...
  test.prop([fc.constant(null)])("all expected categories exist", () => {
    ...
    for (const category of expectedCategories) {
      expect(
        BLEND_MODE_CATEGORIES[category as keyof typeof BLEND_MODE_CATEGORIES],
      ).toBeDefined();
    }
  });
});
