...
calculateGravityForce gravity mass =
  let finiteMass = ensureFinite mass 1.0
      finiteGravityX = ensureFinite (vec2X gravity) 0.0
      finiteGravityY = ensureFinite (vec2Y gravity) 0.0
...
calculateWindForce direction turbulence frequency seed bodyPos frame =
  let finiteTurbulence = ensureFinite turbulence 0.0
      finiteFreq = ensureFinite frequency 1.0
      finiteSeed = ensureFinite seed 0.0
      ...
      dirX = ensureFinite (vec2X direction) 0.0
      dirY = ensureFinite (vec2Y direction) 0.0
      forceX = dirX + noiseX
      forceY = dirY + noiseY
  in Right (Vec2 (ensureFinite forceX 0.0) (ensureFinite forceY 0.0))
...
calculateAttractionForce center bodyPos strength mass radius falloff =
  let diff = vec2Sub center bodyPos
      ...
      dist = safeSqrt distSq
      
      -- Validate distance
      finiteDist = ensureFinite dist 0.0
      finiteRadius = ensureFinite radius 0.0
                    ...
                    then Left "Body is too close to field center (minimum distance: 1)"
                      ...
                      let -- Calculate force magnitude based on falloff
                          forceMag = case falloff of
                            FalloffLinear ->
                              let finiteStrength = ensureFinite strength 0.0
     ...
     then Left "Invalid distance calculation"
          ...
          then Left "Body is outside vortex range"
            ...
            let -- Falloff factor
                ...
                finiteStrength = ensureFinite strength 0.0
                finiteMass = ensureFinite mass 1.0
                ...
                finiteInward = ensureFinite inwardForce 0.0
...
calculateDragForce velocity linear quadratic =
  let speed = vec2Length velocity
      finiteSpeed = ensureFinite speed 0.0
     ...
     then Left "Invalid speed calculation"
          ...
          then Left "Body speed is too low (threshold: 0.01)"
            ...
            let -- Drag magnitude: linear * speed + quadratic * speed^2
                finiteLinear = ensureFinite linear 0.0
                finiteQuadratic = ensureFinite quadratic 0.0
...
calculateBuoyancyForce bodyPos bodyVel surfaceLevel density linearDrag _angularDrag mass radius =
  let bodyY = vec2Y bodyPos
      finiteBodyY = ensureFinite bodyY 0.0
      finiteSurface = ensureFinite surfaceLevel 0.0
      submergedDepth = finiteBodyY - finiteSurface
      finiteDepth = ensureFinite submergedDepth 0.0
     ...
     then Left "Invalid submerged depth calculation"
          ...
          then Left "Body is above water surface"
            ...
            let -- Calculate submerged ratio (approximate based on radius)
                ...
                finiteLinearDrag = ensureFinite linearDrag 0.0
                velX = vec2X bodyVel
                velY = vec2Y bodyVel
                ...
                -- Combined force
                forceX = ensureFinite dragX 0.0
                forceY = ensureFinite (buoyancyForceY + dragY) 0.0
