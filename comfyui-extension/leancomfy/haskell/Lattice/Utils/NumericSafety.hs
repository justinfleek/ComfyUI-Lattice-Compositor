...
module Lattice.Utils.NumericSafety
  ...
  , wrapToRange
  , pingPong
    -- * Plain Double versions (for compatibility)
  , ensureFiniteD
  , clamp01D
  , clampD
  , safeLerpD
...
-- | Alias for isFiniteDouble
isFinite :: Double -> Bool
isFinite = isFiniteDouble

-- | Validate that a Double is finite, returning Either
validateFinite :: Double -> Either String Double
validateFinite x
  | isFiniteDouble x = Right x
  | otherwise = Left "Value is not finite (NaN or Infinity)"
...
--------------------------------------------------------------------------------
-- Plain Double versions (for compatibility with code expecting raw Doubles)
--------------------------------------------------------------------------------

-- | Ensure finite Double, returning fallback Double if not
ensureFiniteD :: Double -> Double -> Double
ensureFiniteD value fallback
  | isFiniteDouble value = value
  | otherwise = fallback

-- | Clamp Double to [0, 1]
clamp01D :: Double -> Double
clamp01D value = max 0 (min 1 value)
...
-- | Safe linear interpolation with Doubles
safeLerpD :: Double -> Double -> Double -> Double
safeLerpD a b t
  | isFiniteDouble result = result
  | isFiniteDouble alt = alt
  | otherwise = a
  where
    tClamped = clamp01D t
    diff = b - a
    result = a + diff * tClamped
    alt = a * (1 - tClamped) + b * tClamped

-- | Safe square root returning Double (0 for negative)
safeSqrtD :: Double -> Double
safeSqrtD value
  | value < 0 = 0
  | isFiniteDouble result && result >= 0 = result
  | otherwise = 0
  where result = sqrt value

-- | Round Double to decimal places
roundToD :: Double -> Int -> Double
roundToD value decimals
  | isFiniteDouble result = result
  | otherwise = value
  where
    factor = 10 ** fromIntegral decimals
...
-- | Safe 2D distance as Double
safeDistanceD :: Double -> Double -> Double -> Double -> Double
safeDistanceD x1 y1 x2 y2 = safeSqrtD (dx * dx + dy * dy)
  where
    dx = x2 - x1
    dy = y2 - y1
