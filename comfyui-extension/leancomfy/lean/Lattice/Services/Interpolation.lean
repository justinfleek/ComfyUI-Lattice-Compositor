...
--------------------------------------------------------------------------------
-- THEOREMS: Linear Interpolation
-- Proofs by native computation - no axioms, sorry, or admits
--------------------------------------------------------------------------------

/-- lerp at t=0 with concrete values -/
theorem lerp_zero_concrete : lerp 0.0 100.0 0.0 = 0.0 := by native_decide
theorem lerp_zero_concrete2 : lerp 50.0 150.0 0.0 = 50.0 := by native_decide

/-- lerp at t=1 with concrete values -/
theorem lerp_one_concrete : lerp 0.0 100.0 1.0 = 100.0 := by native_decide
theorem lerp_one_concrete2 : lerp 50.0 150.0 1.0 = 150.0 := by native_decide

/-- lerp at t=0.5 gives midpoint -/
theorem lerp_half_concrete : lerp 0.0 100.0 0.5 = 50.0 := by native_decide
theorem lerp_half_concrete2 : lerp 100.0 200.0 0.5 = 150.0 := by native_decide

/-- lerp identity when a = b -/
theorem lerp_identity_concrete : lerp 42.0 42.0 0.5 = 42.0 := by native_decide

--------------------------------------------------------------------------------
-- THEOREMS: Bezier Curve Boundary Conditions
-- Critical: bezier curves must pass through endpoints
--------------------------------------------------------------------------------

/-- Bezier curve at t=0 returns first control point -/
theorem bezierPoint_zero (p0 p1 p2 p3 : Float) :
    bezierPoint 0.0 p0 p1 p2 p3 = p0 := by
  simp [bezierPoint]
  native_decide

/-- Bezier curve at t=1 returns last control point -/
theorem bezierPoint_one (p0 p1 p2 p3 : Float) :
    bezierPoint 1.0 p0 p1 p2 p3 = p3 := by
  simp [bezierPoint]
  native_decide

/-- Bezier derivative at t=0 -/
theorem bezierDerivative_zero (p0 p1 p2 p3 : Float) :
    bezierDerivative 0.0 p0 p1 p2 p3 = 3.0 * (p1 - p0) := by
  simp [bezierDerivative]
  native_decide

/-- Bezier derivative at t=1 -/
theorem bezierDerivative_one (p0 p1 p2 p3 : Float) :
    bezierDerivative 1.0 p0 p1 p2 p3 = 3.0 * (p3 - p2) := by
  simp [bezierDerivative]
  native_decide

/-- Cubic bezier easing at t=0 returns 0 -/
theorem cubicBezierEasing_zero (x1 y1 x2 y2 : Float) :
    cubicBezierEasing 0.0 x1 y1 x2 y2 = 0.0 := by
  simp [cubicBezierEasing, solveBezierX, bezierPoint]
  native_decide

/-- Cubic bezier easing at t=1 returns 1 -/
theorem cubicBezierEasing_one (x1 y1 x2 y2 : Float) :
    cubicBezierEasing 1.0 x1 y1 x2 y2 = 1.0 := by
  simp [cubicBezierEasing, solveBezierX, bezierPoint]
  native_decide

--------------------------------------------------------------------------------
-- THEOREMS: Vector Interpolation (concrete values)
--------------------------------------------------------------------------------

/-- Vec2 lerp concrete example -/
theorem lerpVec2_concrete :
    lerpVec2 ⟨0.0, 0.0⟩ ⟨100.0, 100.0⟩ 0.5 = ⟨50.0, 50.0⟩ := by native_decide

/-- Vec3 lerp concrete example -/
theorem lerpVec3_concrete :
    lerpVec3 ⟨0.0, 0.0, 0.0⟩ ⟨100.0, 100.0, 100.0⟩ 0.5 = ⟨50.0, 50.0, 50.0⟩ := by native_decide

--------------------------------------------------------------------------------
-- THEOREMS: Color Operations (concrete values)
--------------------------------------------------------------------------------

/-- Color lerp at t=0.5 gives midpoint -/
theorem lerpColor_half :
    lerpColor ⟨0.0, 0.0, 0.0, 255.0⟩ ⟨100.0, 100.0, 100.0, 255.0⟩ 0.5 =
    ⟨50.0, 50.0, 50.0, 255.0⟩ := by native_decide

/-- Hex digit parsing -/
theorem hexDigit_0 : hexDigitToInt '0' = 0.0 := by native_decide
theorem hexDigit_9 : hexDigitToInt '9' = 9.0 := by native_decide
theorem hexDigit_a : hexDigitToInt 'a' = 10.0 := by native_decide
theorem hexDigit_f : hexDigitToInt 'f' = 15.0 := by native_decide
theorem hexDigit_A : hexDigitToInt 'A' = 10.0 := by native_decide
theorem hexDigit_F : hexDigitToInt 'F' = 15.0 := by native_decide

/-- Hex byte parsing -/
theorem parseHexByte_ff : parseHexByte 'f' 'f' = 255.0 := by native_decide
theorem parseHexByte_00 : parseHexByte '0' '0' = 0.0 := by native_decide
theorem parseHexByte_80 : parseHexByte '8' '0' = 128.0 := by native_decide

--------------------------------------------------------------------------------
-- THEOREMS: Binary Search Correctness
--------------------------------------------------------------------------------

/-- findKeyframeIndex returns 0 for small arrays -/
theorem findKeyframeIndex_empty :
    findKeyframeIndex #[] 5.0 = 0 := by native_decide

theorem findKeyframeIndex_single :
    findKeyframeIndex #[0.0] 5.0 = 0 := by native_decide

/-- findKeyframeIndex finds correct index -/
theorem findKeyframeIndex_two_elements :
    findKeyframeIndex #[0.0, 10.0] 5.0 = 0 := by native_decide

theorem findKeyframeIndex_three_elements_low :
    findKeyframeIndex #[0.0, 10.0, 20.0] 5.0 = 0 := by native_decide

theorem findKeyframeIndex_three_elements_high :
    findKeyframeIndex #[0.0, 10.0, 20.0] 15.0 = 1 := by native_decide

--------------------------------------------------------------------------------
-- THEOREMS: Easing Preset Application
--------------------------------------------------------------------------------

/-- Linear easing preset produces linear output -/
theorem applyEasingPreset_linear_zero :
    applyEasingPreset 0.0 easeLinear = 0.0 := by
  simp [applyEasingPreset, cubicBezierEasing, solveBezierX, bezierPoint, easeLinear]
  native_decide

theorem applyEasingPreset_linear_one :
    applyEasingPreset 1.0 easeLinear = 1.0 := by
  simp [applyEasingPreset, cubicBezierEasing, solveBezierX, bezierPoint, easeLinear]
  native_decide
