...
def Lean.Environment.minimalRequiredModules (env : Environment) : Array Name :=
  let required := Lean.Environment.requiredModules env |>.toArray.erase env.header.mainModule
...
def Lean.Name.findHome (n : Name) (env : Option Environment) : CoreM NameSet := do
  ...
  let required := (← Lean.Name.requiredModules n).toArray.erase current?
...
elab "#import_diff" n:ident* : command => do
  ...
  let reduced_set := NameSet.ofList reduced_all_imports.toList
  let import_diff := extended_all_imports.filter (fun n => !reduced_set.contains n)
