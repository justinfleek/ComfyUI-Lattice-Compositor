...
where
  process (r : NameMap NameSet) (n : Name) (i : Array Name) : NameMap NameSet :=
    if r.contains n then
      ...
      let t := i.foldr
        (fun j s => ((r'.find? j).getD {}).foldl (fun acc x => acc.insert x) s)
        (NameSet.ofList i.toList)
      r'.insert n t
...
def transitiveFilteredUpstream (node : Name) (graph : NameMap (Array Name))
    ...
    List Name :=
  ((graph.find? node).getD #[]).toList.flatMap fun source =>
...
def unusedTransitiveImports (amongst : List Name) (verbose : Bool := false) : CoreM (List (Name × List Name)) := do
  ...
  let transitivelyRequired ← Lean.Environment.transitivelyRequiredModules' env amongst verbose
